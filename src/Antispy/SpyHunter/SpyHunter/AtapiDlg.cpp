...
IMPLEMENT_DYNAMIC(CAtapiDlg, CDialog)

CAtapiDlg::CAtapiDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CAtapiDlg::IDD, pParent)
	, m_szStatus(_T(""))
{
...
void CAtapiDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LIST, m_list);
	DDX_Text(pDX, IDC_ATAPI_STATUS, m_szStatus);
}


BEGIN_MESSAGE_MAP(CAtapiDlg, CDialog)
	ON_BN_CLICKED(IDOK, &CAtapiDlg::OnBnClickedOk)
	ON_WM_SIZE()
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_LIST, &CAtapiDlg::OnNMCustomdrawList)
	ON_NOTIFY(NM_RCLICK, IDC_LIST, &CAtapiDlg::OnNMRclickList)
	ON_COMMAND(ID_SDT_REFRESH, &CAtapiDlg::OnSdtRefresh)
	ON_COMMAND(ID_SDT_ONLY_SHOW_HOOK, &CAtapiDlg::OnSdtOnlyShowHook)
	ON_UPDATE_COMMAND_UI(ID_SDT_ONLY_SHOW_HOOK, &CAtapiDlg::OnUpdateSdtOnlyShowHook)
	ON_COMMAND(ID_SDT_DIS_CURRENT, &CAtapiDlg::OnSdtDisCurrent)
	ON_COMMAND(ID_SDT_DIS_ORIGIN, &CAtapiDlg::OnSdtDisOrigin)
	ON_COMMAND(ID_SDT_RESTORE, &CAtapiDlg::OnSdtRestore)
	ON_COMMAND(ID_SDT_RESTORE_ALL, &CAtapiDlg::OnSdtRestoreAll)
	ON_COMMAND(ID_SDT_SHUXING, &CAtapiDlg::OnSdtShuxing)
	ON_COMMAND(ID_SDT_LOCATION_MODULE, &CAtapiDlg::OnSdtLocationModule)
	ON_COMMAND(ID_SDT_VERIFY_SIGN, &CAtapiDlg::OnSdtVerifySign)
	ON_COMMAND(ID_SDT_TEXT, &CAtapiDlg::OnSdtText)
	ON_COMMAND(ID_SDT_EXCEL, &CAtapiDlg::OnSdtExcel)
	ON_WM_INITMENUPOPUP()
	ON_COMMAND(ID_PROCESS_LOCATION_AT_FILE_MANAGER, &CAtapiDlg::OnProcessLocationAtFileManager)
END_MESSAGE_MAP()

BEGIN_EASYSIZE_MAP(CAtapiDlg)
	EASYSIZE(IDC_LIST, ES_BORDER, ES_BORDER, ES_BORDER, ES_BORDER, 0)
	EASYSIZE(IDC_ATAPI_STATUS, ES_BORDER, ES_KEEPSIZE, ES_BORDER, ES_BORDER, 0)
	EASYSIZE(IDC_STATIC, ES_BORDER, ES_KEEPSIZE, ES_BORDER, ES_BORDER, 0)
END_EASYSIZE_MAP
// CAtapiDlg 消息处理程序

void CAtapiDlg::OnBnClickedOk()
{
}

void CAtapiDlg::OnSize(UINT nType, int cx, int cy)
{
	CDialog::OnSize(nType, cx, cy);
	UPDATE_EASYSIZE;
}

BOOL CAtapiDlg::OnInitDialog()
{
...
void CAtapiDlg::GetDriver()
{
	m_clsDrivers.ListDrivers(m_CommonDriverList);
}

CString CAtapiDlg::GetDriverPath(ULONG pCallback)
{
	CString szPath;
...
void CAtapiDlg::GetDispatch()
{
	OPERATE_TYPE opType = enumAtapiHook;
	BOOL bRet = FALSE;
	ULONG nTotalCount = 0;
	...
	ULONG nSize = 30 * sizeof(DISPATCH_HOOK_INFO);
	...
	DISPATCH_HOOK_INFO StartIoInfo;
	...
	if (bRet)
	{
		...
		if (pFsdHookInfo[0].pNowAddress)
		{
			...
			memcpy(&StartIoInfo, &pFsdHookInfo[28], sizeof(DISPATCH_HOOK_INFO));
		}
	}
	...
	if (StartIoInfo.pNowAddress != 0)
	{
		InsertStartIoDispatch(StartIoInfo);
		nTotalCount++;
	}

	m_szStatus.Format(szAtapiStatus[g_enumLang], nTotalCount, m_nHookedCnt);
	UpdateData(FALSE);
}

void CAtapiDlg::InsertStartIoDispatch(DISPATCH_HOOK_INFO HookInfo)
{
	if (HookInfo.nIndex == 28 && HookInfo.pOriginAddress)
	{
		WCHAR szDriverStartIo[] = {'D','r','i','v','e','r','S','t','a','r','t','I','o','\0'};
		CString szIndex, szOriginalEntry, szCurrentEntry, szHooked, szPath;
		...
		int n = m_list.InsertItem(m_list.GetItemCount(), szIndex);

		if (HookInfo.pOriginAddress != HookInfo.pNowAddress)
		{
			szHooked = L"atapi hook";
			m_nHookedCnt++;
			m_list.SetItemData(n, enumHooked);
		}

		m_list.SetItemText(n, 1, szDriverStartIo);
...
void CAtapiDlg::InsertDispatchItem(DISPATCH_HOOK_INFO HookInfo)
{
		...
		HookInfo.pInlineHookAddress)
	{
		...
		szCurrentEntry.Format(L"(atapi)%s, (inline)%s", szEntry1, szEntry2);
		szPath.Format(L"(atapi)%s, (inline)%s", szPath1, szPath2);
		szHooked = L"atapi & inline";
		bHooked = TRUE;
	}
...
void CAtapiDlg::OnNMCustomdrawList(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CAtapiDlg::OnNMRclickList(NMHDR *pNMHDR, LRESULT *pResult)
{
	CMenu exportto;
...
void CAtapiDlg::OnSdtRefresh()
{
	GetDispatch();
}

void CAtapiDlg::OnSdtOnlyShowHook()
{
	m_bOnlyShowHooks = !m_bOnlyShowHooks;
	OnSdtRefresh();
}

void CAtapiDlg::OnUpdateSdtOnlyShowHook(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(m_bOnlyShowHooks);
}

void CAtapiDlg::OnSdtDisCurrent()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CAtapiDlg::OnSdtRestore()
{
	...
	while (pos)
	{
		...
		if (szHookType.CompareNoCase(L"-"))
		{
			...
			cs.op.Restore.DispatchType = DispatchAtapi;
...
void CAtapiDlg::OnSdtRestoreAll()
{
...
void CAtapiDlg::OnSdtLocationModule()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CAtapiDlg::OnSdtVerifySign()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CAtapiDlg::OnSdtText()
{
	m_Functions.ExportListToTxt(&m_list, m_szStatus);
}

void CAtapiDlg::OnSdtExcel()
{
	WCHAR szAtapi[] = {'A','t','a','p','i','\0'};
	m_Functions.ExportListToExcel(&m_list, szAtapi, m_szStatus);
}

void CAtapiDlg::OnInitMenuPopup(CMenu* pPopupMenu, UINT nIndex, BOOL bSysMenu)
{
...
BOOL CAtapiDlg::PreTranslateMessage(MSG* pMsg)
{
...
void CAtapiDlg::OnProcessLocationAtFileManager()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
