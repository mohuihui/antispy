...
#define TPF_VK           1
#define TPF_ABS          64
#define TPF_EXACT        128		// 只查找展开的键值,跳过例如".", ".."这类的键值
 ...
 */
struct hive {
  char *filename;        /* Hives filename */										/* Hives 文件路径，退出时需要释放资源 */
  int  filedesc;         /* File descriptor (only valid if state == OPEN) */		/*文件描述，是用open函数打开Hive文件时返回的值*/
  int  state;            /* Current state of hive */								/*表明Hive文件是以何种方式打开的，只读或者可读可写等*/
  int  type;             /* Suggested type of hive. NOTE: Library will guess when
			    it loads it, but application may change it if needed */
  int  pages;            /* Number of pages, total */								/*共有几个bin,即几个页*/
  ...
  int  size;             /* Hives size (filesize) in bytes, incl regf header */		/*Hive文件大小*/
  int  rootofs;          /* Offset of root-node */									/*根键的偏移*/
  int  lastbin;          /* Offset to last HBIN */									/*最后一个bin的偏移*/
  int  endofs;           /* Offset of first non HBIN page, we can expand from here */
  short nkindextype;     /* Subkey-indextype the root key uses */					/*这个域表明是什么类型的键巢室 - 子键列表巢室，有一系列的键巢室的巢室索引构成的巢室，这些键巢室是同一个父键下面的所有子键*/
  char *buffer;          /* Files raw contents */									/*Hive文件的内容，已经读取到内存中，退出时需释放*/
...
BOOL WCharToMByte ( IN LPCWSTR lpcwszStr, OUT LPSTR *lpOutBuffer );
BOOL string_is_widechar ( IN PCHAR strKeyName, IN ULONG nLen );
BOOL string_is_unicode ( IN PCHAR strKeyName, IN ULONG nLen );
PVOID change_to_ansi ( IN char* szName, IN DWORD nNameLen );
