...
IMPLEMENT_DYNAMIC(CKbdclassDlg, CDialog)

CKbdclassDlg::CKbdclassDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CKbdclassDlg::IDD, pParent)
	, m_szStatus(_T(""))
{
	m_nHookedCnt = 0;
	m_bOnlyShowHooks = FALSE;
	m_KbdClassHookVector.clear();
	m_CommonDriverList.clear();
}

CKbdclassDlg::~CKbdclassDlg()
{
	m_KbdClassHookVector.clear();
	m_CommonDriverList.clear();
}

void CKbdclassDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LIST, m_list);
	DDX_Text(pDX, IDC_KBDCLASS_STATUS, m_szStatus);
}


BEGIN_MESSAGE_MAP(CKbdclassDlg, CDialog)
	ON_WM_INITMENUPOPUP()
	ON_WM_SIZE()
	ON_BN_CLICKED(IDOK, &CKbdclassDlg::OnBnClickedOk)
	ON_NOTIFY(NM_RCLICK, IDC_LIST, &CKbdclassDlg::OnNMRclickList)
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_LIST, &CKbdclassDlg::OnNMCustomdrawList)
	ON_COMMAND(ID_SDT_REFRESH, &CKbdclassDlg::OnSdtRefresh)
	ON_COMMAND(ID_SDT_ONLY_SHOW_HOOK, &CKbdclassDlg::OnSdtOnlyShowHook)
	ON_UPDATE_COMMAND_UI(ID_SDT_ONLY_SHOW_HOOK, &CKbdclassDlg::OnUpdateSdtOnlyShowHook)
	ON_COMMAND(ID_SDT_DIS_CURRENT, &CKbdclassDlg::OnSdtDisCurrent)
	ON_COMMAND(ID_SDT_DIS_ORIGIN, &CKbdclassDlg::OnSdtDisOrigin)
	ON_COMMAND(ID_SDT_RESTORE, &CKbdclassDlg::OnSdtRestore)
	ON_COMMAND(ID_SDT_RESTORE_ALL, &CKbdclassDlg::OnSdtRestoreAll)
	ON_COMMAND(ID_SDT_SHUXING, &CKbdclassDlg::OnSdtShuxing)
	ON_COMMAND(ID_SDT_LOCATION_MODULE, &CKbdclassDlg::OnSdtLocationModule)
	ON_COMMAND(ID_SDT_VERIFY_SIGN, &CKbdclassDlg::OnSdtVerifySign)
	ON_COMMAND(ID_SDT_TEXT, &CKbdclassDlg::OnSdtText)
	ON_COMMAND(ID_SDT_EXCEL, &CKbdclassDlg::OnSdtExcel)
	ON_COMMAND(ID_PROCESS_LOCATION_AT_FILE_MANAGER, &CKbdclassDlg::OnProcessLocationAtFileManager)
END_MESSAGE_MAP()

BEGIN_EASYSIZE_MAP(CKbdclassDlg)
	EASYSIZE(IDC_LIST, ES_BORDER, ES_BORDER, ES_BORDER, ES_BORDER, 0)
	EASYSIZE(IDC_KBDCLASS_STATUS, ES_BORDER, ES_KEEPSIZE, ES_BORDER, ES_BORDER, 0)
	EASYSIZE(IDC_STATIC, ES_BORDER, ES_KEEPSIZE, ES_BORDER, ES_BORDER, 0)
END_EASYSIZE_MAP
// CKbdclassDlg 消息处理程序

void CKbdclassDlg::OnBnClickedOk()
{
}

void CKbdclassDlg::OnInitMenuPopup(CMenu* pPopupMenu, UINT nIndex, BOOL bSysMenu)
{
...
void CKbdclassDlg::OnSize(UINT nType, int cx, int cy)
{
	CDialog::OnSize(nType, cx, cy);
	UPDATE_EASYSIZE;
}

BOOL CKbdclassDlg::OnInitDialog()
{
...
void CKbdclassDlg::GetDriver()
{
	m_clsDrivers.ListDrivers(m_CommonDriverList);
}

CString CKbdclassDlg::GetDriverPath(ULONG pCallback)
{
	CString szPath;
...
void CKbdclassDlg::GetKbdclassDispatch()
{
	OPERATE_TYPE opType = enumKbdClassHook;
	BOOL bRet = FALSE;
	ULONG nTotalCount = 0;
	...
	if (bRet)
	{
		...
		if (pFsdHookInfo[0].pNowAddress)
		{
			...
			{
				m_KbdClassHookVector.push_back(pFsdHookInfo[i]);
			}
		}
	}
	...
	for (vector<DISPATCH_HOOK_INFO>::iterator itor = m_KbdClassHookVector.begin(); itor != m_KbdClassHookVector.end(); itor++)
	{
		InsertKbdclassItem(*itor);
		nTotalCount++;
	}

	m_szStatus.Format(szKdbStatus[g_enumLang], nTotalCount, m_nHookedCnt);
	UpdateData(FALSE);
}

void CKbdclassDlg::InsertKbdclassItem(DISPATCH_HOOK_INFO HookInfo)
{
		...
		HookInfo.pInlineHookAddress)
	{
		...
		szCurrentEntry.Format(L"(kbdclass)%s, (inline)%s", szEntry1, szEntry2);
		szPath.Format(L"(kbdclass)%s, (inline)%s", szPath1, szPath2);
		szHookType = L"kbdclass & inline";
		bHooked = TRUE;
	}
...
void CKbdclassDlg::OnNMRclickList(NMHDR *pNMHDR, LRESULT *pResult)
{
	CMenu exportto;
...
void CKbdclassDlg::OnNMCustomdrawList(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CKbdclassDlg::OnSdtRefresh()
{
	GetKbdclassDispatch();
}

void CKbdclassDlg::OnSdtOnlyShowHook()
{
	m_bOnlyShowHooks = !m_bOnlyShowHooks;
	OnSdtRefresh();
}

void CKbdclassDlg::OnUpdateSdtOnlyShowHook(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(m_bOnlyShowHooks);
}

void CKbdclassDlg::OnSdtDisCurrent()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CKbdclassDlg::OnSdtDisOrigin()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CKbdclassDlg::OnSdtRestore()
{
	POSITION pos = m_list.GetFirstSelectedItemPosition();
	...
	while (pos)
	{
		...
		if (szHookType.CompareNoCase(L"-"))
		{
			...
			cs.op.Restore.DispatchType = DispatchKbdclass;
...
void CKbdclassDlg::OnSdtRestoreAll()
{
...
void CKbdclassDlg::OnSdtShuxing()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CKbdclassDlg::OnSdtLocationModule()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CKbdclassDlg::OnSdtVerifySign()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CKbdclassDlg::OnSdtText()
{
	m_Functions.ExportListToTxt(&m_list, m_szStatus);
}

void CKbdclassDlg::OnSdtExcel()
{
	WCHAR szKeyboard[] = {'K','e','y','b','o','a','r','d','\0'};
	m_Functions.ExportListToExcel(&m_list, szKeyboard, m_szStatus);
}

BOOL CKbdclassDlg::PreTranslateMessage(MSG* pMsg)
{
...
void CKbdclassDlg::OnProcessLocationAtFileManager()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
