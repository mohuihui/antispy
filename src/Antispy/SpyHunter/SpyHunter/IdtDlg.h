...
class CIdtDlg : public CDialog
{
	DECLARE_EASYSIZE
	DECLARE_DYNAMIC(CIdtDlg)

public:
	CIdtDlg(CWnd* pParent = NULL);   // 标准构造函数
	virtual ~CIdtDlg();

// 对话框数据
	enum { IDD = IDD_IDT_DIALOG };
...
public:
	...
	void GetIdt();
	void InsertIdtItem(IDT_HOOK_INFO HookInfo);
	vector<IDT_HOOK_INFO> m_IdtHookVector;
