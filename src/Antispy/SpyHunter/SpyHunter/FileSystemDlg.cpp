...
IMPLEMENT_DYNAMIC(CFileSystemDlg, CDialog)

CFileSystemDlg::CFileSystemDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CFileSystemDlg::IDD, pParent)
	, m_szStatus(_T(""))
{
	m_NtfsHookVector.clear();
	m_FastfatHookVector.clear();
...
void CFileSystemDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_FILE_SYSTEM_CNT, m_szStatus);
}
...
BEGIN_MESSAGE_MAP(CFileSystemDlg, CDialog)
	...
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_LIST, &CFileSystemDlg::OnNMCustomdrawList)
	ON_COMMAND(ID_SDT_REFRESH, &CFileSystemDlg::OnSdtRefresh)
	ON_COMMAND(ID_SDT_ONLY_SHOW_HOOK, &CFileSystemDlg::OnSdtOnlyShowHook)
	ON_UPDATE_COMMAND_UI(ID_SDT_ONLY_SHOW_HOOK, &CFileSystemDlg::OnUpdateSdtOnlyShowHook)
	ON_COMMAND(ID_SDT_DIS_CURRENT, &CFileSystemDlg::OnSdtDisCurrent)
	ON_COMMAND(ID_SDT_DIS_ORIGIN, &CFileSystemDlg::OnSdtDisOrigin)
	ON_COMMAND(ID_SDT_RESTORE, &CFileSystemDlg::OnSdtRestore)
	ON_COMMAND(ID_SDT_RESTORE_ALL, &CFileSystemDlg::OnSdtRestoreAll)
	ON_COMMAND(ID_SDT_SHUXING, &CFileSystemDlg::OnSdtShuxing)
	ON_COMMAND(ID_SDT_LOCATION_MODULE, &CFileSystemDlg::OnSdtLocationModule)
	ON_COMMAND(ID_SDT_VERIFY_SIGN, &CFileSystemDlg::OnSdtVerifySign)
	ON_COMMAND(ID_SDT_TEXT, &CFileSystemDlg::OnSdtText)
	ON_COMMAND(ID_SDT_EXCEL, &CFileSystemDlg::OnSdtExcel)
	ON_COMMAND(ID_PROCESS_LOCATION_AT_FILE_MANAGER, &CFileSystemDlg::OnProcessLocationAtFileManager)
END_MESSAGE_MAP()

BEGIN_EASYSIZE_MAP(CFileSystemDlg)
	EASYSIZE(IDC_LIST, ES_BORDER, ES_BORDER, ES_BORDER, ES_BORDER, 0)
	EASYSIZE(IDC_FILE_SYSTEM_CNT, ES_BORDER, ES_KEEPSIZE, ES_BORDER, ES_BORDER, 0)
...
void CFileSystemDlg::OnSize(UINT nType, int cx, int cy)
{
...
void CFileSystemDlg::OnInitMenuPopup(CMenu* pPopupMenu, UINT nIndex, BOOL bSysMenu)
{
...
void CFileSystemDlg::OnNMCustomdrawList(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CFileSystemDlg::GetFsdDispatch()
{
	OPERATE_TYPE opType = enumFsdHooks;
	ULONG nCnt = 28 * 3;
	BOOL bRet = FALSE;
	ULONG nTotalCount = 0;
	...
	ULONG nSize = nCnt * sizeof(DISPATCH_HOOK_INFO);
	...
	if (bRet)
	{
		...
		if (pFsdHookInfo[0].pNowAddress)
		{
			...
			{
				m_NtfsHookVector.push_back(pFsdHookInfo[i]);
			}
		}

		if (pFsdHookInfo[28].pNowAddress)
		{
			for (ULONG i = 28; i < 28 * 2; i++)
			{
				m_FastfatHookVector.push_back(pFsdHookInfo[i]);
			}
		}
	}
	...
	for (vector<DISPATCH_HOOK_INFO>::iterator itor = m_NtfsHookVector.begin(); itor != m_NtfsHookVector.end(); itor++)
	{
		InsertFsdItem(*itor, enumNtfs);
		nTotalCount++;
	}

	for (vector<DISPATCH_HOOK_INFO>::iterator itor = m_FastfatHookVector.begin(); itor != m_FastfatHookVector.end(); itor++)
	{
		InsertFsdItem(*itor, enumFastfat);
		nTotalCount++;
	}

	m_szStatus.Format(szFsdStatus[g_enumLang], nTotalCount, m_nHookedCnt);
	UpdateData(FALSE);
}

void CFileSystemDlg::InsertFsdItem(DISPATCH_HOOK_INFO HookInfo, FILE_SYSTEM_TYPE Type)
{
	CString szHookType, szCurrentEntry, szIndex, szOriginalEntry, szFunctionName, szPath;
	BOOL bHooked = FALSE;
	int n = 0;

	if (Type == enumNtfs)
	{
		szFunctionName = L"(NTFS)";
	}
	else if (Type == enumFastfat)
	{
		szFunctionName = L"(Fastfat)";
	}

 	szFunctionName += szDispatchName[HookInfo.nIndex];
	szIndex.Format(L"%d", HookInfo.nIndex);
	szOriginalEntry.Format(L"0x%08X", HookInfo.pOriginAddress);

	// 即是fsd hook，又是inline hook
	if (HookInfo.pNowAddress != HookInfo.pOriginAddress &&
		HookInfo.pInlineHookAddress)
	{
		CString szInlineEntryTemp;
		...
		szCurrentEntry.Format(L"(fsd)%s, (inline)%s", szEntry1, szEntry2);
		szPath.Format(L"(fsd)%s, (inline)%s", szPath1, szPath2);
		szHookType = L"fsd & inline";
		bHooked = TRUE;
	}
	// fsd hook
	else if (HookInfo.pNowAddress != HookInfo.pOriginAddress)
	{
		...
		if (szPath.IsEmpty())
		{
			...
			for (int i = 0; i < 5; i++)
			{
				...
				{
					szPath = szPathTemp;
					szCurrentEntry += szInlineEntryTemp;
					break;
				}
	...
	if (m_bOnlyShowHooks)
	{
		if (bHooked)
		{
			...
			m_list.SetItemText(n, 1, szFunctionName);
...
void CFileSystemDlg::GetDriver()
{
	m_clsDrivers.ListDrivers(m_CommonDriverList);
}

CString CFileSystemDlg::GetDriverPath(ULONG pCallback)
{
	CString szPath;
...
void CFileSystemDlg::OnSdtRefresh()
{
	GetFsdDispatch();
}

void CFileSystemDlg::OnSdtOnlyShowHook()
{
	m_bOnlyShowHooks = !m_bOnlyShowHooks;
	GetFsdDispatch();
}

void CFileSystemDlg::OnUpdateSdtOnlyShowHook(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(m_bOnlyShowHooks);
}

void CFileSystemDlg::OnSdtDisCurrent()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CFileSystemDlg::OnSdtDisOrigin()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CFileSystemDlg::OnSdtRestore()
{
	POSITION pos = m_list.GetFirstSelectedItemPosition();
	...
	while (pos)
	{
		...
		if (szHookType.CompareNoCase(L"-"))
		{
			...
			CString szFunctionName = m_list.GetItemText(nItem, 1);
			DISPATCH_TYPE DispatchType = DispatchUnKnow;
			DISPATCH_HOOK cs;
			WCHAR szNtfs[] = {'(','N','T','F','S',')','\0'};
			WCHAR szFastfat[] = {'(','F','a','s','t','\0'};

			if (!_wcsnicmp(szFunctionName.GetBuffer(), szNtfs, wcslen(szNtfs)))
			{
				DispatchType = DispatchNtfs;
			}
			else if (!_wcsnicmp(szFunctionName.GetBuffer(), szFastfat, wcslen(szFastfat)))
			{
				DispatchType = DispatchFastfat;
			}

			cs.OpType = enumRestoreDispatchHook;
			cs.op.Restore.DispatchType = DispatchType;
...
void CFileSystemDlg::OnSdtRestoreAll()
{
	...
	for (int i = 0; i < nCnt; i++)
	{
		...
		if (szHookType.CompareNoCase(L"-"))
		{
			...
			ULONG nIndex = _wtoi(szIndex);
			CString szFunctionName = m_list.GetItemText(i, 1);
			DISPATCH_TYPE DispatchType = DispatchUnKnow;
			DISPATCH_HOOK cs;
			WCHAR szNtfs[] = {'(','N','T','F','S',')','\0'};
			WCHAR szFastfat[] = {'(','F','a','s','t','\0'};
...
void CFileSystemDlg::OnSdtShuxing()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CFileSystemDlg::OnSdtLocationModule()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CFileSystemDlg::OnSdtVerifySign()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CFileSystemDlg::OnSdtText()
{
	m_Functions.ExportListToTxt(&m_list, m_szStatus);
}

void CFileSystemDlg::OnSdtExcel()
{
	WCHAR szFSD[] = {'F','S','D','\0'};
	m_Functions.ExportListToExcel(&m_list, szFSD, m_szStatus);
}
...
void CFileSystemDlg::OnProcessLocationAtFileManager()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
