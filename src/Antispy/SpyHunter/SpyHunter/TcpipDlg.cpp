...
void CTcpipDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_TCPIP_STATUS, m_szStatus);
...
BEGIN_MESSAGE_MAP(CTcpipDlg, CDialog)
	...
	ON_WM_SIZE()
	ON_NOTIFY(NM_RCLICK, IDC_LIST, &CTcpipDlg::OnNMRclickList)
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_LIST, &CTcpipDlg::OnNMCustomdrawList)
	ON_COMMAND(ID_SDT_REFRESH, &CTcpipDlg::OnSdtRefresh)
	ON_COMMAND(ID_SDT_ONLY_SHOW_HOOK, &CTcpipDlg::OnSdtOnlyShowHook)
	ON_UPDATE_COMMAND_UI(ID_SDT_ONLY_SHOW_HOOK, &CTcpipDlg::OnUpdateSdtOnlyShowHook)
	ON_COMMAND(ID_SDT_DIS_CURRENT, &CTcpipDlg::OnSdtDisCurrent)
	ON_COMMAND(ID_SDT_DIS_ORIGIN, &CTcpipDlg::OnSdtDisOrigin)
	ON_COMMAND(ID_SDT_RESTORE, &CTcpipDlg::OnSdtRestore)
	ON_COMMAND(ID_SDT_RESTORE_ALL, &CTcpipDlg::OnSdtRestoreAll)
	ON_COMMAND(ID_SDT_SHUXING, &CTcpipDlg::OnSdtShuxing)
	ON_COMMAND(ID_SDT_LOCATION_MODULE, &CTcpipDlg::OnSdtLocationModule)
	ON_COMMAND(ID_SDT_VERIFY_SIGN, &CTcpipDlg::OnSdtVerifySign)
	ON_COMMAND(ID_SDT_TEXT, &CTcpipDlg::OnSdtText)
	ON_COMMAND(ID_SDT_EXCEL, &CTcpipDlg::OnSdtExcel)
	ON_WM_INITMENUPOPUP()
	ON_COMMAND(ID_PROCESS_LOCATION_AT_FILE_MANAGER, &CTcpipDlg::OnProcessLocationAtFileManager)
END_MESSAGE_MAP()

BEGIN_EASYSIZE_MAP(CTcpipDlg)
	EASYSIZE(IDC_LIST, ES_BORDER, ES_BORDER, ES_BORDER, ES_BORDER, 0)
	EASYSIZE(IDC_TCPIP_STATUS, ES_BORDER, ES_KEEPSIZE, ES_BORDER, ES_BORDER, 0)
...
void CTcpipDlg::OnSize(UINT nType, int cx, int cy)
{
...
void CTcpipDlg::GetDriver()
{
	m_clsDrivers.ListDrivers(m_CommonDriverList);
}

CString CTcpipDlg::GetDriverPath(ULONG pCallback)
{
	CString szPath;
...
void CTcpipDlg::GetDispatch()
{
	OPERATE_TYPE opType = enumTcpipHook;
	BOOL bRet = FALSE;
	ULONG nTotalCount = 0;
	...
	m_szStatus.Format(szTcpipStatus[g_enumLang], nTotalCount, m_nHookedCnt);
	UpdateData(FALSE);
}

void CTcpipDlg::InsertDispatchItem(DISPATCH_HOOK_INFO HookInfo)
{
		...
		HookInfo.pInlineHookAddress)
	{
		...
		szCurrentEntry.Format(L"(tcpip)%s, (inline)%s", szEntry1, szEntry2);
		szPath.Format(L"(tcpip)%s, (inline)%s", szPath1, szPath2);
		szHooked = L"tcpip & inline";
		bHooked = TRUE;
	}
...
void CTcpipDlg::OnNMRclickList(NMHDR *pNMHDR, LRESULT *pResult)
{
	CMenu exportto;
...
void CTcpipDlg::OnNMCustomdrawList(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CTcpipDlg::OnSdtRefresh()
{
	GetDispatch();
}

void CTcpipDlg::OnSdtOnlyShowHook()
{
	m_bOnlyShowHooks = !m_bOnlyShowHooks;
	OnSdtRefresh();
}

void CTcpipDlg::OnUpdateSdtOnlyShowHook(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(m_bOnlyShowHooks);
}

void CTcpipDlg::OnSdtDisCurrent()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CTcpipDlg::OnSdtRestore()
{
	...
	while (pos)
	{
		...
		if (szHookType.CompareNoCase(L"-"))
		{
			...
			cs.op.Restore.DispatchType = DispatchTcpip;
...
void CTcpipDlg::OnSdtRestoreAll()
{
...
void CTcpipDlg::OnSdtLocationModule()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CTcpipDlg::OnSdtVerifySign()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
...
void CTcpipDlg::OnSdtText()
{
	m_Functions.ExportListToTxt(&m_list, m_szStatus);
}

void CTcpipDlg::OnSdtExcel()
{
	WCHAR szTcpip[] = {'T','c','p','i','p','\0'};
	m_Functions.ExportListToExcel(&m_list, szTcpip, m_szStatus);
}

void CTcpipDlg::OnInitMenuPopup(CMenu* pPopupMenu, UINT nIndex, BOOL bSysMenu)
{
...
BOOL CTcpipDlg::PreTranslateMessage(MSG* pMsg)
{
...
void CTcpipDlg::OnProcessLocationAtFileManager()
{
	int nItem = m_Functions.GetSelectItem(&m_list);
	if (nItem != -1)
	{
