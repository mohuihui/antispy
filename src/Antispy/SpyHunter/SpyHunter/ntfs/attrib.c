...
#pragma warning( disable: 4244 4013 4761 )
...
{								
	...
	return (na->ni->flags & FILE_ATTR_COMPRESSED);	
	return 0;						
}		
...
{								
	if (na->type == AT_DATA && na->name == AT_UNNAMED) {	
		...
		ntfs_log_trace("BUG! Should be called only for unnamed data attribute.\n");	
	}
...
{								
	if (na->type == AT_DATA && na->name == AT_UNNAMED)	
		return (na->ni->flags & FILE_ATTR_ENCRYPTED);	
	return 0;						
}		
...
{								
	if (na->type == AT_DATA && na->name == AT_UNNAMED) {	
		na->ni->flags |= FILE_ATTR_ENCRYPTED;		
	} else {						
		ntfs_log_trace("BUG! Should be called only for unnamed data attribute.\n");	
	}
...
{								
	if (na->type == AT_DATA && na->name == AT_UNNAMED)	
		return (na->ni->flags & FILE_ATTR_SPARSE_FILE);	
	return 0;						
}		
...
ntfs_get_attribute_value_length(
	IN const ATTR_RECORD *a
	)
/*++

Leaner: sudami [sudami@163.com]
Time  : 2008/10/22 [22:10:2008 - 11:36]

Routine Description:
  得到该属性的大小.常驻属性为value_length;非常驻属性为data_size;
...
--*/
{
	...
	errno = 0;
	if (a->non_resident)
		return sle64_to_cpu(a->X.data_size);
	
	return (s64)le32_to_cpu(a->X.value_length);
}
...
Time  : 2009/03/02 [2:3:2009 - 20:13]

Routine Description:
  Get a copy of an attribute  
...
--*/
{
	...
	if ( !a->non_resident )  // 1. 常驻属性
	{
		// 此属性长度的合法性判断
		if (le32_to_cpu(a->X.value_length) + le16_to_cpu(a->X.value_offset)
				> le32_to_cpu(a->length)) {
			return 0;
		}

		// 把这个属性(除去属性头)的内容拷贝一份到buffer中
		memcpy( b, (const char*)a + le16_to_cpu(a->X.value_offset), le32_to_cpu(a->X.value_length) );
		errno = 0;
	...
	{
		if ( total + (rl[i].length << vol->cluster_size_bits) >= sle64_to_cpu(a->X.data_size) ) 
		{
			UCHAR *intbuf = NULL;
			/*
			 ...
			 */
			intbuf = (UCHAR *)ntfs_malloc(rl[i].length << vol->cluster_size_bits);
			if (!intbuf) {
				free(rl);
				return 0;
			}
			...
			r = ntfs_pread(
				vol->dev, 
				rl[i].lcn << vol->cluster_size_bits,    // 从该卷的哪里开始读
				rl[i].length << vol->cluster_size_bits, // 要读多少内容出来
				intbuf );								// 保存读出的内容
...
#undef ESTR
			...
			// 拷贝读到的内容到Buffer中
			memcpy( b + total, intbuf, sle64_to_cpu(a->X.data_size) - total );
			free( intbuf );
			total = sle64_to_cpu( a->X.data_size );
			break;
		}
		/*
		 * FIXME: If compressed file: Only read if lcn != -1.
		...
		ntfschar *name, u32 name_len)
{
	...
	if (a->non_resident) {
		ntfs_attr_init(na, TRUE, a->flags & ATTR_IS_COMPRESSED,
				...
				sle64_to_cpu(a->X.allocated_size),
				sle64_to_cpu(a->X.data_size),
				sle64_to_cpu(a->X.initialized_size),
				cs ? sle64_to_cpu(a->X.compressed_size) : 0,
				cs ? a->X.compression_unit : 0);
	} else {
		s64 l = le32_to_cpu(a->X.value_length);
...
Author: sudami [sudami@163.com]
Time  : 2009/03/02 [2:3:2009 - 16:17]

Routine Description:
   map (a part of) a runlist of an ntfs attribute    
...
--*/
{
	...
	ntfs_log_trace(
		...
		(ULONG64)na->ni->mft_no, 
		na->type, 
		(LONG64)vcn );

	// 在该数据运行列表中查找出VCN对应的LCN
	lcn = ntfs_rl_vcn_to_lcn( na->rl, vcn );
	if (lcn >= 0 || lcn == LCN_HOLE || lcn == LCN_ENOENT) { return 0; }

	//
	// 没有找到对应的LCN. 分配&初始化一个查找结构体
	//
	ctx = ntfs_attr_get_search_ctx( na->ni, NULL );
	if (!ctx) { return -1; }

	// 在MFT中找到该属性. return 0 表示成功
	   ...
	   ) 
	{
		...
		// Decode the runlist.
		rl = ntfs_mapping_pairs_decompress( na->ni->vol, ctx->attr, na->rl );
		if (rl) 
		{
			na->rl = rl;
			ntfs_attr_put_search_ctx( ctx );
...
int ntfs_attr_map_whole_runlist(ntfs_attr *na)
{
	...
	while (1) {
		...
		if (!next_vcn) {
			...
			last_vcn = sle64_to_cpu(a->X.allocated_size) >>
					vol->cluster_size_bits;
		}

		/* Get the lowest vcn for the next extent. */
		highest_vcn = sle64_to_cpu(a->X.highest_vcn);
		...
		if (next_vcn < sle64_to_cpu(a->X.lowest_vcn)) {
			errno = EIO;
	...
	if (highest_vcn && highest_vcn != last_vcn - 1) {
		...
		ntfs_log_perror("Couldn't load full runlist: "
				...
				(LONG64)highest_vcn, (LONG64)last_vcn);
		goto err_out;
	}
	err = errno;
	ntfs_attr_put_search_ctx(ctx);
...
Author: sudami [sudami@163.com]
Time  : 2009/03/02 [2:3:2009 - 10:28]

Routine Description:
  在@na对应的属性的run list中查找对应的VCN.
...
--*/
{
	...
	if (!na || !NAttrNonResident(na) || vcn < 0) // 参数合法性检测
	{
		errno = EINVAL;
		return NULL;
	}
	...
	ntfs_log_trace(
		"Entering for inode 0x%llx, attr 0x%x, vcn %llx\n",
		(ULONG64)na->ni->mft_no, na->type,
		(LONG64)vcn );

retry:
	rl = na->rl;
	if (!rl) { goto map_rl; } // 若该属性的数据运行还没有dump,则dump过来后再查找
	if (vcn < rl[0].vcn) { goto map_rl; }

	while ( rl->length ) 
	{
		if ( vcn < rl[1].vcn ) 
		{
			// 若该VCN在当前的这个数据运行块中,则成功返回
			if ( rl->lcn >= (LCN)LCN_HOLE ) { return rl; }
			break;
		}
...
map_rl:
	...
	if (is_retry || errno == EINVAL || errno == ENOENT) { errno = EIO; }
	return NULL;
}
...
Author: sudami [sudami@163.com]
Time  : 2009/03/02 [2:3:2009 - 9:31]

Routine Description:
  从属性@na中指定的偏移@pos处开始读取@count的内容,放入指针@b中  
...
--*/
{
	...
	ntfs_log_trace(
		...
		(ULONG64)na->ni->mft_no,
		na->type, (LONG64)pos, (LONG64)count
		);
	...
	//更新访问时间
	if ( na->type == AT_DATA || na->type == AT_INDEX_ROOT || na->type == AT_INDEX_ALLOCATION ) {
		ntfs_inode_update_atime( na->ni );
	}

	if (!count) { return 0; }

	// 若要读的数据超过了该属性实际大小,会截断处理
	if ( pos + count > na->data_size ) 
	{
		if ( pos >= na->data_size ) { return 0; }
	...
	if ( !NAttrNonResident( na ) ) 
	{
		...
		// 在该MFT中查找对应属性,保存至ctx. return 0表示成功
		if ( ntfs_attr_lookup( na->type, na->name, na->name_len, 0,0, NULL, 0, ctx ) ) 
		{
res_err_out:
			ntfs_attr_put_search_ctx( ctx ); // 释放掉刚才分配的内存
			return -1;
		}

		// 得到该属性的属性内容地址,合法性检测
		val = (char*)ctx->attr + le16_to_cpu(ctx->attr->X.value_offset);
		if (val < (char*)ctx->attr 
			|| val + le32_to_cpu(ctx->attr->X.value_length) >(char*)ctx->mrec + vol->mft_record_size
		   ) 
		{
			errno = EIO;
			goto res_err_out;
		...
		//
		// 读取常驻属性的关键一句: 将该属性内容偏移@pos后的@count字节的数据拷贝到@b指针指向的Buffer中
		//

		memcpy( b, val + pos, count );
		ntfs_attr_put_search_ctx(ctx);  // 释放掉刚才分配的内存
		return count; // 返回已经读取并拷贝过的数据大小
	}
	...
	{
		if ( pos >= na->initialized_size ) { // 参数合法性检测
			memset(b, 0, count);
			return count;
		}

		// 若该属性中的数据没那么大,你却要读取比其大得多的数据,则将超过的部分初始化为0
	...
	for ( ; count; rl++, ofs = 0 ) 
	{
		...
		if ( !rl->length ) { goto rl_err_out; }
		if ( rl->lcn < (LCN)0 ) // 还有逻辑簇号为负数的情况吗?应该是旧的NTFS上的特性?
		{
			if (rl->lcn != (LCN)LCN_HOLE) { goto rl_err_out; }
...
retry:
		ntfs_log_trace("Reading 0x%llx bytes from vcn 0x%llx, lcn 0x%llx, ofs 0x%llx.\n", to_read, rl->vcn, rl->lcn, ofs);
	...
	)
{
	...
	if ( !NAttrNonResident(na) ) 
	{
		...
		if (ntfs_attr_lookup(na->type, na->name, na->name_len, 0, 0, NULL, 0, ctx)) { goto err_out; }
	...
	if (pos + count > na->initialized_size) 
	{
		if (ntfs_attr_map_whole_runlist(na)) { goto err_out; }
		...
		ctx->attr->X.initialized_size = cpu_to_sle64(pos + count);
	...
	if (need_to.undo_initialized_size) {
		...
		if (!err) {
			...
			if (!err) {
				...
				ctx->attr->X.initialized_size = cpu_to_sle64(
...
--*/
{
	...
	for ( end = (u8*)dst + br * bk_size; (u8*)dst < end; dst = (u8*)dst + bk_size )
		...
		const u32 bk_size, void *src)
{
	...
	for ( i = 0; i < bk_cnt; ++i )
	{
		...
		err = ntfs_mst_pre_write_fixup( (NTFS_RECORD*)((u8*)src + i * bk_size), bk_size );
		if (err < 0) {
			if (!i) return err;
			bk_cnt = i;
			break;
	...
	for (i = 0; i < bk_cnt; ++i)
		ntfs_mst_post_write_fixup( (NTFS_RECORD*)((u8*)src + i * bk_size) );
...
ntfs_attr_find(
	...
	IN const ntfschar *name,
	IN const u32 name_len,
	IN const IGNORE_CASE_BOOL ic,
	IN const u8 *val, 
	IN const u32 val_len, 
	IN ntfs_attr_search_ctx *ctx
	)
/*++

Author: sudami [sudami@163.com]
Time  : 2009/03/01 [1:3:2009 - 1:13]

Routine Description:

  请勿直接调用此函数,换用lookup_attr(). 
  ...
  characters. If @name is not present (NULL), we assume that the unnamed attribute
  is being searched for. Finally, the resident attribute value @val is looked for,
  if present.
...
Arguments:
  ...
  type - 要查找的属性类型.eg. 90H 对应 AT_INDEX_ROOT
  name - 要查找的属性名.eg. "$I30"(optional, i.e. NULL means don't care)
  name_len - 要查找的属性名长度
  ic - IGNORE_CASE or CASE_SENSITIVE
...
Return Value:
  ...
  若查找成功,则返回0,且@ctx->attr指向这个已找到的属性; 若未找到,则返回-1.
  若@ctx->is_first 是TRUE,表明是第一次查找,则从@ctx->attr本身开始找,否则
  在@ctx->attr之后找
...
--*/
{
	...
	for ( ;;	a = (ATTR_RECORD*)((char*)a + le32_to_cpu(a->length)) ) 
	{
		if ( p2n(a) < p2n(ctx->mrec) 
			|| (char*)a > (char*)ctx->mrec + *(PSHORT)((char*)ctx->mrec + 28) /*le32_to_cpu(ctx->mrec->bytes_allocated*/
		   )
		{ 
			// 检查该属性的合法性
			break; 
		...
		if ( !a->length ) { break; }		
		if ( type == AT_UNUSED ) { return 0; } // If this is an enumeration return this attribute.
		if ( a->type != type ) { continue; } // 不是匹配的属性,就继续下一个遍历

		if (name == AT_UNNAMED) {

			if (a->name_length) { // The search failed if the found attribute is named.
				errno = ENOENT;
				return -1;
			}
		} else if ( name 
			&& !ntfs_names_are_equal( name, name_len, 
			(ntfschar*)((char*)a + /*le16_to_cpu*/(a->name_offset)),a->name_length, 
			...
			)
		{
			...
			rc = ntfs_names_collate( name, name_len,
					(ntfschar*)((char*)a + /*le16_to_cpu*/(a->name_offset)), a->name_length, 
					1, IGNORE_CASE, upcase, upcase_len );
			...
			rc = ntfs_names_collate( name, name_len,
					(ntfschar*)((char*)a + /*le16_to_cpu*/(a->name_offset)), a->name_length,
					1, CASE_SENSITIVE, upcase, upcase_len );
		...
		if ( !val ) {
			...
			rc = memcmp(
					val, 
					(char*)a +le16_to_cpu(a->X.value_offset),
					min(val_len,le32_to_cpu(a->X.value_length)) );
		
			if (!rc) 
			{
				register u32 avl;
				avl = le32_to_cpu(a->X.value_length);
				if ( val_len == avl ) return 0;
				if ( val_len < avl ) {
					errno = ENOENT;
					return -1;
				}
...
Author: sudami [sudami@163.com]
Time  : 2009/03/01 [1:3:2009 - 10:54]

Routine Description:
  find an attribute in the attribute list of an inode. 在0x20H这个属性链表中查找属性
  不要直接调用此函数,换用 ntfs_attr_lookup()
...
--*/
{
	...
	ntfs_log_trace("Entering for inode 0x%llx, attribute type 0x%x.\n",(ULONG64)ni->mft_no, type);

	if (!base_ni) // 没有父MFT,表明这个就是根目录的MFT,是第一次查找
	{ 
		base_ni = ctx->base_ntfs_ino = ctx->ntfs_ino; // 全部指向自己
		ctx->base_mrec = ctx->mrec;
	}

	if (ni == base_ni) { ctx->base_attr = ctx->attr; }	// 属性记录也指向自己
	if (type == AT_END) { goto not_found; }				// 类型不正确,则退出查找

	vol = base_ni->vol;
	al_start = base_ni->attr_list;				 // 该MFT中属性链表的开始地址
	al_end = al_start + base_ni->attr_list_size; // 该MFT中属性链表的结束地址
	if ( !ctx->al_entry ) 
	{
	...
	if ( ctx->is_first ) // 处理第一次查找的情况
	{
		 ...
		 */
		if ((type == AT_UNUSED) 
			&& le32_to_cpu(ctx->al_entry->type) < le32_to_cpu(AT_ATTRIBUTE_LIST) 
			&& le32_to_cpu(al_entry->type) > le32_to_cpu(AT_ATTRIBUTE_LIST)
		   )
		{
			int rc;
...
find_attr_list_attr:
			...
			ctx->attr	  = (ATTR_RECORD*)((u8*)ctx->mrec + le16_to_cpu(ctx->mrec->attrs_offset));

			// Find the attribute list attribute. 
			rc = ntfs_attr_find( AT_ATTRIBUTE_LIST, NULL, 0, IGNORE_CASE, NULL, 0, ctx );
	...
	//
	// 遍历attrbute_list数据块,得到每个在一个MFT中装不下的属性
	//
	for ( ;; al_entry = next_al_entry )
	{
		if ( (u8*)al_entry < base_ni->attr_list || (u8*)al_entry > al_end ) { break; }

		ctx->al_entry = al_entry;
		if ( (u8*)al_entry == al_end ) { goto not_found; } // 查找到头了.返回之
		if ( !al_entry->length ) { break; } // 若当前的属性链表当前数据块没有长度了,表明属性链表正式结束
		if ( (u8*)al_entry + 6 > al_end || (u8*)al_entry + le16_to_cpu(al_entry->length) > al_end ) { break; }

		next_al_entry = (ATTR_LIST_ENTRY*)((u8*)al_entry + le16_to_cpu(al_entry->length)); // 下一个attrbute_list数据块
		if ( type != AT_UNUSED ) 
		{
			if (le32_to_cpu(al_entry->type) > le32_to_cpu(type)) { goto not_found; }
			if (type != al_entry->type) { continue; } // 当前的属性链表当前数据块中的type类型不匹配,继续遍历下一个
		}

		// OK,在当前的属性链表当前数据块中找到匹配的了,但不一定匹配名字,进一步验证
		al_name_len = al_entry->name_length;
		al_name = (ntfschar*)((u8*)al_entry + al_entry->name_offset);
	
		if (type == AT_UNUSED) { goto is_enumeration; } // 不是查找特定的属性,而是列举该MFT的所有属性
	
		if (name == AT_UNNAMED) {
			if (al_name_len) { goto not_found; }
				...
				vol->upcase_len))
		{
			...
			if (rc == -1) { goto not_found; }
			if (rc) { continue; } // 若名字不相同,继续下一个查找
		...
		if (lowest_vcn 
			...
			&& (u8*)next_al_entry + le16_to_cpu(next_al_entry->length) <= al_end
			&& sle64_to_cpu(next_al_entry->lowest_vcn) <= lowest_vcn
			&& next_al_entry->type == al_entry->type
			&& next_al_entry->name_length == al_name_len
			&& ntfs_names_are_equal(
					(ntfschar*)((char*)next_al_entry + next_al_entry->name_offset),
...
is_enumeration:
		//
		// MREF_LE宏的作用是去掉文件MFT参考号的高4位,即去掉其顺序号,保留其文件号
		// 因为一个文件MFT参考号是2个DWORD
		//
		if ( MREF_LE(al_entry->mft_reference) == ni->mft_no )
		{
			if ( MSEQNO_LE(al_entry->mft_reference) != le16_to_cpu( ni->mrec->sequence_number ) ) {
				ntfs_log_debug("Found stale mft reference in attribute list!\n");
				break;
			}

		} else { // Mft references do not match. Do we want the base record back? 
			...
			if ( MREF_LE(al_entry->mft_reference) == base_ni->mft_no ) {
				...
				ni = ntfs_extent_inode_open( base_ni, al_entry->mft_reference );
				if (!ni) 
				{
...
do_next_attr_loop:
		...
		if (al_entry->type != a->type)
			break;
		if (!ntfs_names_are_equal((ntfschar*)((char*)a + le16_to_cpu(a->name_offset)),
		...
		if ((type == AT_UNUSED) || !val || (!a->non_resident &&
				le32_to_cpu(a->X.value_length) == val_len &&
				!memcmp((char*)a + le16_to_cpu(a->X.value_offset),
				val, val_len))
		   ) 
		{
			return 0;
...
not_found:
	...
	if (type == AT_UNUSED || type == AT_END) {
		ntfs_attr_reinit_search_ctx(ctx);
		return ntfs_attr_find(AT_END, name, name_len, ic, val, val_len, ctx);
	}
...
Author: sudami [sudami@163.com]
Time  : 2009/03/01 [1:3:2009 - 0:36]

Routine Description:

  在inode中找到指定类型的属性
  首次查找时,@ctx->ntfs_ino 必须是 the base mft record.而且参数一必须已经被初始化过了
  ( 通过调用ntfs_attr_get_search_ctx() ).

  如果要查找的属性类型@type为AT_UNUSED,则返回第一个查找到的属性. 我们可以为该函数
  ntfs_attr_lookup()传递此参数,然后循环调用它,来遍历该MFT的所有属性,直到其返回 -1.
  在循环中,每次成功的调用会返回下一个属性的地址,而把当前查找到的属性地址放入到结构体
  @ctx 中.

  如果要查找的属性类型@type为AT_END,则定位到该MFT的结束处,并返回-1. It should never 
  be needed to do this, but we implement the functionality because it allows for 
  simpler code inside ntfs_external_attr_find().

  当完成了操作后,务必要调用函数 ntfs_attr_put_search_ctx() 来做清理工作.
  若函数调用成功,则 @ctx->attr 保存的是要查找的属性,@ctx->mrec 是该MFT记录,and 
  @ctx->al_entry is the attribute list entry for this attribute with @ctx->base_* 
  being the base mft record to which @ctx->attr belongs. If no attribute list attribute 
  is present @ctx->al_entry and @ctx->base_* are NULL.
...
Return Value:
  Return 0 if the search was successful and -1 if not, with errno set to the error code.
...
--*/
{
	...
	if (!ctx || !ctx->mrec || !ctx->attr || 
		...
		(!ctx->ntfs_ino || !(vol = ctx->ntfs_ino->vol) || !vol->upcase || !vol->upcase_len)
		)
	   ) 
	{
		errno = EINVAL;
	...
	if ( !base_ni || !NInoAttrList(base_ni) || type == AT_ATTRIBUTE_LIST ) 
	{
		...
		// 查找扩展属性
		return ntfs_external_attr_find( type, name, name_len, ic, lowest_vcn, val, val_len, ctx );
	}
}
...
--*/
{
	if (!mrec) { mrec = ni->mrec; }
	ctx->mrec = mrec; // MFT头
	ctx->attr = (ATTR_RECORD*)((u8*)mrec + le16_to_cpu(mrec->attrs_offset)); // MFT头过后,第一个属性的位置
	ctx->is_first		= TRUE	;
	ctx->ntfs_ino		= ni	;
	ctx->al_entry		= NULL	;
	ctx->base_ntfs_ino	= NULL	;
...
Author: sudami [sudami@163.com]
Time  : 2009/03/03 [3:3:2009 - 16:43]
...
--*/
{
	...
	ctx = (ntfs_attr_search_ctx*)ntfs_malloc( sizeof(ntfs_attr_search_ctx) );
	if ( ctx ) { ntfs_attr_init_search_ctx( ctx, ni, mrec ); }
	...
	)
{
	...
	for ( ad = vol->attrdef; (u8*)ad - (u8*)vol->attrdef < vol->attrdef_len && ad->type; ++ad ) 
	{
...
Author: sudami [sudami@163.com]
Time  : 2009/03/06 [6:3:2009 - 13:33]

Routine Description:
  check a size of an attribute type for validity'
  若要扩充一个属性的大小,必须在$ATTRIBUTE_LIST中检查合法性
...
--*/
{
	...
	if ((sle64_to_cpu(ad->min_size) && size < sle64_to_cpu(ad->min_size)) 
		|| ((sle64_to_cpu(ad->max_size) > 0) && size > sle64_to_cpu(ad->max_size))
	   ) 
	{
...
int ntfs_make_room_for_attr(MFT_RECORD *m, u8 *pos, u32 size)
{
	...
	ntfs_log_trace("Entering for pos 0x%d, size %u.\n", (int)(pos - (u8*)m), (ULONG64) size);

	size = (size + 7) & ~7;

	/* Rigorous consistency checks. */
	if (!m || !pos || pos < (u8*)m || pos + size > (u8*)m + le32_to_cpu(m->bytes_allocated)) {
		errno = EINVAL;
		return -1;
	}
	...
	)
{
	 ...
	 * file record.
	 */
	if (!ntfs_attr_find(type, name, name_len, CASE_SENSITIVE, val, size, ctx)) {
		err = EEXIST;
	...
	length = offsetof(ATTR_RECORD, X.resident_end) +
	...
	a->name_offset = cpu_to_le16(offsetof(ATTR_RECORD, X.resident_end));
	a->flags = flags;
	a->instance = m->next_attr_instance;
	a->X.value_length = cpu_to_le32(size);
	a->X.value_offset = cpu_to_le16(length - ((size + 7) & ~7));
	if (val)
		memcpy((u8*)a + le16_to_cpu(a->X.value_offset), val, size);
	else
		memset((u8*)a + le16_to_cpu(a->X.value_offset), 0, size);
	if (type == AT_FILE_NAME)
		a->X.resident_flags = RESIDENT_ATTR_IS_INDEXED;
	else
		a->X.resident_flags = 0;
	if (name_len)
	...
	if (type != AT_ATTRIBUTE_LIST && NInoAttrList(base_ni)) 
	{
		if (ntfs_attrlist_entry_add(ni, a)) {
			...
			ntfs_log_trace("Failed add attribute entry to ATTRIBUTE_LIST.\n");
			goto put_err_out;
		}
	}
	...
	)
{
	...
	ntfs_log_trace("Entering for inode 0x%llx, attr 0x%x, lowest_vcn %lld, "
			...
			(LONG64) ni->mft_no, (ULONG64) type,
			(LONG64) lowest_vcn, dataruns_size, (ULONG64) flags);
	 ...
	 * file record.
	 */
	if (!ntfs_attr_find(type, name, name_len, CASE_SENSITIVE, NULL, 0, ctx)) {
		err = EEXIST;
		ntfs_log_trace("Attribute already present.\n");
		goto put_err_out;
	}
	...
	length = offsetof(ATTR_RECORD, X.compressed_size) + ((sizeof(ntfschar) *
			name_len + 7) & ~7) + dataruns_size +
			((flags & (ATTR_IS_COMPRESSED | ATTR_IS_SPARSE)) ?
			sizeof(a->X.compressed_size) : 0);
	...
	a->non_resident = 1;
	a->name_length = name_len;
	a->name_offset = cpu_to_le16(offsetof(ATTR_RECORD, X.compressed_size) +
			((flags & (ATTR_IS_COMPRESSED | ATTR_IS_SPARSE)) ?
			sizeof(a->X.compressed_size) : 0));
	a->flags = flags;
	a->instance = m->next_attr_instance;
	a->X.lowest_vcn = cpu_to_sle64(lowest_vcn);
	a->X.mapping_pairs_offset = cpu_to_le16(length - dataruns_size);
	a->X.compression_unit = (flags & ATTR_IS_COMPRESSED) ? 4 : 0;
	/* If @lowest_vcn == 0, than setup empty attribute. */
	if (!lowest_vcn) {
		a->X.highest_vcn = cpu_to_sle64(-1);
		a->X.allocated_size = 0;
		a->X.data_size = 0;
		a->X.initialized_size = 0;
		/* Set empty mapping pairs. */
		*((u8*)a + le16_to_cpu(a->X.mapping_pairs_offset)) = 0;
	}
	 ...
	 */
	ntfs_attr_reinit_search_ctx(ctx);
	if (ntfs_attr_lookup(type, name, name_len, CASE_SENSITIVE, lowest_vcn, NULL, 0, ctx)) {
...
int ntfs_attr_record_rm(ntfs_attr_search_ctx *ctx)
{
	...
	ntfs_log_trace("Entering for inode 0x%llx, attr 0x%x.\n",
			(LONG64) ctx->ntfs_ino->mft_no,
			(ULONG64) le32_to_cpu(ctx->attr->type));
	type = ctx->attr->type;
	ni = ctx->ntfs_ino;
	if (ctx->base_ntfs_ino)
		base_ni = ctx->base_ntfs_ino;
	...
	if (NInoAttrList(base_ni) && type != AT_ATTRIBUTE_LIST)
	{
		if (ntfs_attrlist_entry_rm(ctx)) {
			ntfs_log_trace("Couldn't delete record from $ATTRIBUTE_LIST.\n");
			return -1;
		}
	}
	...
	if (le32_to_cpu(ctx->mrec->bytes_in_use) - le16_to_cpu(ctx->mrec->attrs_offset) == 8) 
	{
	...
	{
		ntfs_attr_reinit_search_ctx(ctx);
		if (ntfs_attr_lookup(AT_ATTRIBUTE_LIST, NULL, 0, CASE_SENSITIVE,0, NULL, 0, ctx))
		{
			/*
			 ...
			 */
			ntfs_log_trace("Couldn't find attribute list. Succeed anyway.\n");
			return 0;
		}
		...
		if (ctx->attr->non_resident) 
		{
			...
			al_rl = ntfs_mapping_pairs_decompress(base_ni->vol,	ctx->attr, NULL);
			if (!al_rl) {
				ntfs_log_trace("Couldn't decompress attribute list runlist. Succeed anyway.\n");
				return 0;
			}

			if (ntfs_cluster_free_from_rl(base_ni->vol, al_rl)) {
				ntfs_log_trace("Leaking clusters! Run chkdsk. Couldn't free clusters from attribute list runlist.\n");
			}
		...
		if (ntfs_attr_record_rm(ctx)) 
		{
			 ...
			 * but without extents.
			 */
			ntfs_log_trace("Couldn't remove attribute list. Succeed anyway.\n");
			return 0;
		}
	}
		...
		ntfschar *name, u8 name_len, u8 *val, s64 size)
{
	...
	if (is_resident)
		attr_rec_size = offsetof(ATTR_RECORD, X.resident_end) +
				((name_len * sizeof(ntfschar) + 7) & ~7) +
				((size + 7) & ~7);
	else
		attr_rec_size = offsetof(ATTR_RECORD, X.non_resident_end) + ((name_len * sizeof(ntfschar) + 7) & ~7) + 8;
	...
	for (i = 0; i < ni->nr_extents; i++) 
	{
		...
		if (le32_to_cpu(attr_ni->mrec->bytes_allocated) - le32_to_cpu(attr_ni->mrec->bytes_in_use) >= attr_rec_size)
			goto add_attr_record;
	}
...
add_attr_record:
	if (is_resident) {
		...
		offset = ntfs_resident_attr_record_add(attr_ni, type, name, name_len, val, size, 0);
		if (offset < 0) {
			err = errno;
	...
	offset = ntfs_non_resident_attr_record_add(attr_ni, type, name, name_len, 0, 8, 0);
	if (offset < 0) {
		err = errno;
	...
	if (ntfs_attr_truncate(na, size) || (val && (ntfs_attr_pwrite(na, 0, size, val) != size))) 
	{
		err = errno;
		ntfs_log_trace("Failed to initialize just added attribute.\n");
		if (ntfs_attr_rm(na))
			ntfs_log_trace("Failed to remove just added attribute. Probably leaving inconstant metadata.\n");
		ntfs_attr_close(na);
		goto err_out;
	}
	ntfs_attr_close(na);
...
rm_attr_err_out:
	...
	if (ntfs_attr_record_resize(attr_ni->mrec, (ATTR_RECORD*)((u8*)attr_ni->mrec + offset), 0)) 
	{
...
free_err_out:
	/* Free MFT record, if it isn't contain attributes. */
	if (le32_to_cpu(attr_ni->mrec->bytes_in_use) - le32_to_cpu(attr_ni->mrec->attrs_offset) == 8) 
	{
		if (ntfs_mft_record_free(attr_ni->vol, attr_ni)) {
			ntfs_log_trace("Failed to free MFT record. Leaving inconstant metadata.\n");
		}
	}
err_out:
	errno = err;
...
int ntfs_attr_rm(ntfs_attr *na)
{
	...
	if (!ctx)
		return -1;
	while (!ntfs_attr_lookup(na->type, na->name, na->name_len, CASE_SENSITIVE, 0, NULL, 0, ctx)) 
	{
		if (ntfs_attr_record_rm(ctx)) {
...
Author: sudami [sudami@163.com]
Time  : 2009/03/06 [6:3:2009 - 14:11]
...
--*/
{
	...
	old_size   = le32_to_cpu(m->bytes_in_use);		// 本MFT已使用的大小
	alloc_size = le32_to_cpu(m->bytes_allocated);	// 本MFT的总分配大小
	attr_size  = le32_to_cpu(a->length);			// 本MFT中该属性的大小
	
	ntfs_log_trace("Sizes: old=%u alloc=%u attr=%u new=%u\n", (ULONG64)old_size, (ULONG64)alloc_size, (ULONG64)attr_size, (ULONG64)new_size);

	// 8 Bytes对齐
	new_size = (new_size + 7) & ~7;
	
	// 若本属性的大小被改变了,需要同时更新一些东西
	if ( new_size != attr_size ) 
	{
		// 重新计算"本MFT已使用的大小"
		u32 new_muse = old_size - attr_size + new_size;
		
		// 若这个长度超过了本MFT的分配长度,肯定不行呀
		if ( new_muse > alloc_size ) {
			errno = ENOSPC;
			ntfs_log_trace("Not enough space in the MFT record (%u > %u)\n", new_muse, alloc_size);
			return -1;
		}
		
		// 因为本属性的大小已经改变,所以需要将本属性后面的内容移动一下.
		memmove( 
			(u8 *)a + new_size, 
			(u8 *)a + attr_size,
...
ntfs_resident_attr_value_resize(
	IN MFT_RECORD *m, 
	IN ATTR_RECORD *a,
	IN const u32 new_size
	)
/*++

Author: sudami [sudami@163.com]
Time  : 2009/03/06 [6:3:2009 - 14:06]

Routine Description:
  重新整合本MFT的内容(常驻属性),因为其中的一个属性的大小变动了,该MFT中其后
  的所有属性内容都会被相应的挪动,并且对于那个变动的属性新多出的部分要清零.


    本MFT(Orignal)                本MFT(Updated)

  +--------------+               +--------------+    
  |	   属性 1	 |               |	   属性 1	|
   --------------   现在属性2     --------------
  |	   属性 2	 | ------------> |	   属性 2	|
   --------------   被扩充了     |xxxxxxxxxxxxxx| ----> 这部分扩充的,就用0填充
  |	    ...	     | -+            |xxxxxxxxxxxxxx|       (这仅仅是扩充的情况,缩减的情况一样处理)
   --------------   |             --------------        
  |	   属性 N	 |  +-------->   |	    ...	    | 
  +--------------+  移动到这里了  -------------- 
								 |	   属性 N	|
...
--*/
{
	...
	if ( ntfs_attr_record_resize(m, a, (le16_to_cpu(a->X.value_offset) + new_size + 7) & ~7 ) < 0 ) { 
		return -1; 
	}
	
	// 若新大小 大于 该属性的原始大小,那么需要将多出的部分内容清零
	if (new_size > le32_to_cpu(a->X.value_length)) 
	{
		memset(
			(u8*)a + le16_to_cpu(a->X.value_offset) + le32_to_cpu(a->X.value_length), 
			0, 
			new_size - le32_to_cpu(a->X.value_length)
		);
	}

	// 更新该属性的大小
	a->X.value_length = cpu_to_le32(new_size);
...
int ntfs_attr_record_move_to(ntfs_attr_search_ctx *ctx, ntfs_inode *ni)
{
	...
	{
		ntfs_log_trace("Attribute of such type, with same name already present in this MFT record.\n");
		err = EEXIST;
		goto put_err_out;
	}
	...
	if (ntfs_make_room_for_attr(ni->mrec, (u8*) nctx->attr, le32_to_cpu(a->length)))
	{
		err = errno;
	...
	nctx->mrec->next_attr_instance = cpu_to_le16((le16_to_cpu(nctx->mrec->next_attr_instance) + 1) & 0xffff);
	ntfs_attr_record_resize(ctx->mrec, a, 0);
	ntfs_inode_mark_dirty(ctx->ntfs_ino);
	ntfs_inode_mark_dirty(ni);

	/* Update attribute list. */
	ctx->al_entry->mft_reference = MK_LE_MREF(ni->mft_no, le16_to_cpu(ni->mrec->sequence_number));
...
Author: sudami [sudami@163.com]
Time  : 2009/03/06 [6:3:2009 - 15:16]
...
--*/
{
	...
	// 确保该属性是 "常驻属性"
	if ( NAttrNonResident( na ) ) {
		ntfs_log_trace("Eeek!  Trying to make non-resident attribute non-resident.  Aborting...\n");
		errno = EINVAL;
		return -1;
	}

	// 确保该属性可以变成 "非常驻属性"
	if (ntfs_attr_can_be_non_resident(vol, na->type)) { return -1; }

	new_allocated_size = ( le32_to_cpu(a->X.value_length) + vol->cluster_size - 1 ) & ~(vol->cluster_size - 1);
	...
	if (new_allocated_size > 0) 
	{
		...
		rl = ntfs_cluster_alloc( 
					vol, 0, 
					new_allocated_size >> vol->cluster_size_bits, // 需要多少个簇
					-1, DATA_ZONE 
					);

		if (!rl) 
		{
			if (errno != ENOSPC) ntfs_log_trace("Eeek!  Failed to allocate cluster(s).  Aborting...\n");
			return -1;
		}
	...
	na->data_size = na->initialized_size = le32_to_cpu(a->X.value_length);
	...
	if (rl) 
	{
		// 拷贝原来常驻属性的内容到run list指向的内存中来
		bw = ntfs_attr_pwrite( na, 0, le32_to_cpu(a->X.value_length), (u8*)a + le16_to_cpu(a->X.value_offset) );
		if ( bw != le32_to_cpu(a->X.value_length) ) 
		{
			err = errno;
			ntfs_log_debug("Eeek!  Failed to write out attribute value (bw = %lli, errno = %i). Aborting...\n", (LONG64)bw, err);
			if (bw >= 0) { err = EIO; }
	...
	if ( mp_size < 0 ) {
		err = errno;
		ntfs_log_debug("Eeek!  Failed to get size for mapping pairs array.  Aborting...\n");
		goto cluster_free_err_out;
	}

	/* Calculate new offsets for the name and the mapping pairs array. */
	name_ofs = (sizeof(ATTR_REC) - sizeof(a->X.compressed_size) + 7) & ~7;
	...
	a->X.lowest_vcn = cpu_to_sle64(0);
	a->X.highest_vcn = cpu_to_sle64((new_allocated_size - 1) >>
						vol->cluster_size_bits);

	a->X.mapping_pairs_offset = cpu_to_le16(mp_ofs);

	a->X.compression_unit = 0;

	memset(&a->X.reserved1, 0, sizeof(a->X.reserved1));

	a->X.allocated_size = cpu_to_sle64(new_allocated_size);
	a->X.data_size = a->X.initialized_size = cpu_to_sle64(na->data_size);
...
ntfs_resident_attr_resize(
	IN ntfs_attr *na, 
	IN const s64 newsize
	)
/*++

Author: sudami [sudami@163.com]
Time  : 2009/03/06 [6:3:2009 - 13:29]

Routine Description:
  resize a resident, open ntfs attribute  
...
--*/
{
	...
	if ( ntfs_attr_size_bounds_check(vol, na->type, newsize) < 0 ) 
	{
		err = errno;
		if (err == ERANGE) {
			ntfs_log_trace("Eeek!  Size bounds check failed. Aborting...\n");
		} else if (err == ENOENT)
			err = EIO;
		goto put_err_out;
	}
	...
	if ( newsize < vol->mft_record_size ) 
	{
		if ( !ntfs_resident_attr_value_resize(ctx->mrec, ctx->attr,newsize) ) 
		{
			...
			if ( NAttrCompressed(na) || NAttrSparse(na) ) { na->compressed_size = na->allocated_size; }
	...
	//
	// NewSize > 一个MFT记录的大小. 需要用到run list
	//
	if ( !ntfs_attr_make_non_resident( na, ctx ) ) 
	{
		// 成功的将 "常驻属性" 变成 "非常驻属性".现在标记为dirty
		ntfs_inode_mark_dirty( ctx->ntfs_ino );
		ntfs_attr_put_search_ctx(ctx);
		...
		err = errno;
		ntfs_log_trace("Eeek!  Failed to make attribute non-resident. Aborting...\n");
		goto put_err_out;
	}
	...
	while ( !ntfs_attr_lookup(AT_UNUSED, NULL, 0, 0, 0, NULL, 0, ctx) ) 
	{
		...
		if (le32_to_cpu(a->length) <= offsetof(ATTR_RECORD, X.compressed_size) + ((a->name_length * sizeof(ntfschar) + 7) & ~7) + 8)
			continue;

		tna = ntfs_attr_open(na->ni, a->type, (ntfschar*)((u8*)a + le16_to_cpu(a->name_offset)), a->name_length);
		if (!tna) {
			err = errno;
	...
	{
		ntfs_attr_put_search_ctx( ctx );
		if ( ntfs_inode_free_space(na->ni, offsetof(ATTR_RECORD, X.non_resident_end) + 8) ) 
		{
...
static int ntfs_attr_make_resident(ntfs_attr *na, ntfs_attr_search_ctx *ctx)
{
	...
	if (sle64_to_cpu(a->X.lowest_vcn)) {
	...
	a->non_resident = 0;
	a->flags = 0;
	a->X.value_length = cpu_to_le32(na->data_size);
	a->X.value_offset = cpu_to_le16(val_ofs);
	/*
...
retry:
	...
	while (!ntfs_attr_lookup(na->type, na->name, na->name_len,
		...
		if (from_vcn) {
			...
			stop_vcn = sle64_to_cpu(a->X.lowest_vcn);
			from_vcn = 0;
			/*
		...
		if (finished_build) {
			...
			a->X.highest_vcn = cpu_to_sle64(NTFS_VCN_DELETE_MARK);
			ntfs_inode_mark_dirty(ctx->ntfs_ino);
			continue;
		}
		...
		if (!a->X.lowest_vcn) {
			...
			a->X.allocated_size = cpu_to_sle64(na->allocated_size);
			/* Update sparse bit. */
			...
			if (sparse && !(a->flags & (ATTR_IS_SPARSE |
				...
				NAttrSetSparse(na);
				a->flags |= ATTR_IS_SPARSE;
				a->X.compression_unit = 4; /* Windows set it so,
							    even if attribute
							    is not actually
							    compressed. */
			...
			if (sparse) {
				...
				a->X.compressed_size = cpu_to_sle64(new_compr_size);
			}
			/*
		 ...
		 */
		cur_max_mp_size = le32_to_cpu(a->length) - le16_to_cpu(a->X.mapping_pairs_offset);
		/*
		...
		{
			if (ntfs_attr_record_resize(m, a, le16_to_cpu(a->X.mapping_pairs_offset) + mp_size))
			{
		...
		/* Update lowest vcn. */
		a->X.lowest_vcn = cpu_to_sle64(stop_vcn);
		ntfs_inode_mark_dirty(ctx->ntfs_ino);
		if ((ctx->ntfs_ino->nr_extents == -1 ||
		...
		if (!ntfs_mapping_pairs_build(na->ni->vol, (u8*)a + le16_to_cpu(
				a->X.mapping_pairs_offset), mp_size, na->rl,
				stop_vcn, &stop_vcn))
			finished_build = TRUE;
		...
		a->X.highest_vcn = cpu_to_sle64(stop_vcn - 1);
	}
	...
	if (finished_build) {
		...
		{
			if (sle64_to_cpu(ctx->attr->X.highest_vcn) != NTFS_VCN_DELETE_MARK)
				continue;
	...
	while (1) {
		...
		cur_max_mp_size = le32_to_cpu(m->bytes_allocated) -
				le32_to_cpu(m->bytes_in_use) -
				(offsetof(ATTR_RECORD, X.compressed_size) +
		...
		err = ntfs_mapping_pairs_build(na->ni->vol, (u8*)a +
			le16_to_cpu(a->X.mapping_pairs_offset), mp_size, na->rl,
			stop_vcn, &stop_vcn);
...
static int ntfs_non_resident_attr_shrink(ntfs_attr *na, const s64 newsize)
{
	...
	ntfs_log_trace("Inode 0x%llx attr 0x%x new size %lld\n", (ULONG64)
		       na->ni->mft_no, na->type, (LONG64)newsize);
	...
	ctx->attr->X.data_size = cpu_to_sle64(newsize);
	if (newsize < na->initialized_size) {
		na->initialized_size = newsize;
		ctx->attr->X.initialized_size = cpu_to_sle64(newsize);
	}
...
static int ntfs_non_resident_attr_expand(ntfs_attr *na, const s64 newsize)
{
	...
	ntfs_log_trace("Inode 0x%llx, attr 0x%x, new size %lld old size %lld\n",
			(ULONG64)na->ni->mft_no, na->type,
			(LONG64)newsize, (LONG64)na->data_size);
...
Author: sudami [sudami@163.com]
Time  : 2009/03/06 [6:3:2009 - 13:15]

Routine Description:
  resize an ntfs attribute  
...
--*/
{
	...
	// 参数合法性检测
	if ( !na || newsize < 0 || (na->ni->mft_no == FILE_MFT && na->type == AT_DATA) ) 
	{
		ntfs_log_trace("Invalid arguments passed.\n");
		errno = EINVAL;
		return STATUS_ERROR;
	...
	if ( NAttrNonResident(na) ) {
		...
		if ( newsize > na->data_size ) {
			ret = ntfs_non_resident_attr_expand( na, newsize ); // 扩大
		} else {
			ret = ntfs_non_resident_attr_shrink( na, newsize ); // 缩减
		}
...
Author: sudami [sudami@163.com]
Time  : 2009/03/03 [3:3:2009 - 16:32]

Routine Description:
  读取inode节点中指定的属性内容,分配新内存存储之,并返回这个指针. 
  最后个参数保存该属性内容的大小  
...
--*/
{
	...
	// 分配内存
	data = (void *)ntfs_malloc(na->data_size);
	if (!data) { goto out; }
...
int ntfs_attr_remove(ntfs_inode *ni, const ATTR_TYPES type, ntfschar *name,  u32 name_len)
{
	ntfs_attr *na;
	int ret;
	...
	na = ntfs_attr_open(ni, type, name, name_len);
	if (!na) {
		ntfs_log_perror("Failed to open attribute 0x%02x of inode 0x%llx", type, (ULONG64)ni->mft_no);
		return -1;
	}
	
	ret = ntfs_attr_rm(na);
	if (ret)
		ntfs_log_perror("Failed to remove attribute 0x%02x of inode 0x%llx", type, (ULONG64)ni->mft_no);
	ntfs_attr_close(na);
