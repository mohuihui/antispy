...
static void 
ntfs_rl_mc(
	runlist_element *dstbase, 
	int dst,
	runlist_element *srcbase, 
	int src, 
	int size
	)
{
...
Time  : 2009/03/02 [2:3:2009 - 16:27]
...
--*/
{
	...
	if (old_size == new_size) { return rl; }
...
runlist_element *
ntfs_runlists_merge(
	IN runlist_element *drl,
	IN runlist_element *srl
	)
/*++

Author: sudami [sudami@163.com]
Time  : 2009/03/02 [2:3:2009 - 15:34]
...
Routine Description:
  ...
  It can also be appended to the end of the runlist, which is just a variant(变种) of the insert case.
...
--*/
{
	...
	while (srl[si].length && srl[si].lcn < (LCN)LCN_HOLE) { si++; }

	// Can't have an entirely unmapped source runlist.
	if (!srl[si].length) 
	{
		ntfs_log_debug("Eeek! ntfs_runlists_merge() received entirely unmapped source runlist.\n");
		errno = EINVAL;
		return NULL;
	}
	...
	for (; drl[di].length; di++) {
		if (drl[di].vcn + drl[di].length > srl[sstart].vcn) { break; }
	}
...

	{
		...
		if (finish && !drl[dins].length) { ss++; }
		if (marker && (drl[dins].vcn + drl[dins].length > srl[send - 1].vcn)) { finish = FALSE; }
		...
		if (marker) {
			...
			if (drl[ds].vcn <= marker_vcn) {
				...
				if (drl[ds].vcn == marker_vcn) {
					ntfs_log_debug(
						"Old marker = %lli, replacing with LCN_ENOENT.\n",
						(LONG64)drl[ds].lcn );
...
Author: sudami [sudami@163.com]
Time  : 2009/03/02 [2:3:2009 - 13:45]
...
--*/
{
	...
	u16 rlpos;				/* Current runlist position in units of runlist_elements. */
	...
	// 确保属性存在,且是非常驻属性
	if (!attr || !attr->non_resident || sle64_to_cpu(attr->X.lowest_vcn) < (VCN)0) 
	{
		errno = EINVAL;
		return NULL;
	}
	...
	vcn = sle64_to_cpu(attr->X.lowest_vcn); // 起始VCN
	vcn = *(VCN*)( (char*)attr + 16 );
	lcn = 0;

	buf = (const u8*)attr + /**(PUSHORT)( (char*)attr + 32 )*/ (attr->X.mapping_pairs_offset); // 定位到数据运行信息处(run)
	attr_end = (const u8*)attr + le32_to_cpu(attr->length); // 该属性结束地址

	if (buf < (const u8*)attr || buf > attr_end) // 参数合法性检测
	{
	...
	// 分配一块4KB大小的内存
	rlsize = 0x1000;
	rl = (runlist_element *) ntfs_malloc(rlsize);
	if ( !rl ) { return NULL; }

	//
	// Insert unmapped starting element if necessary.
	// 比如这个数据运行的起始VCN就是7,那么在7之前的那块就用 LCN_RL_NOT_MAPPED 标记一下
	//
	if ( vcn ) 
	{
		rl->vcn = (VCN)0;
	...
	//
	// 循环读取run list中的内容,存储到自定义结构体runlist_element 中
	//
	while ( buf < attr_end && *buf ) 
	{
		...
		rl[rlpos].vcn = vcn; // ----- 记录1 -----
	
		b = *buf & 0xf;
		//
		// 取出低4位,表示"该运行所占用的簇数在该压缩数据中占用的字节数"
		// 哎,真绕口,举个例子就明晰了:
		//  31H 25H 07H 00 02 00 01 00H
		//  |
		//  1是其低4位,表明是后面一个字节的内容 -- 25H,说明该运行所占用
		//   的簇数为25H个簇
		//  3是其高4位,表示运行的起始簇号; 这里是25H后的3字节内容,即是 --
		//  07H 00 02,也就是从020007开始
		//
		// -- sudami 08/10/22
		if ( b ) 
		{
			if (buf + b > attr_end) { goto io_error; }

			// 这个循环是把 "运行占用的簇数" 取出来,保存到deltaxcn中
			for (deltaxcn = (s8)buf[b--]; b; b--) {
				deltaxcn = (deltaxcn << 8) + buf[b];
			}

		} else { // The length entry is compulsory(强制性的).
		...
		rl[rlpos].length = deltaxcn; // ----- 记录2 -----
		
		//
		// 这时候就指向下一个VCN了.(下一个运行时)
		//
		vcn += deltaxcn;
		if ( !(*buf & 0xf0) ) {
			...
			//
			u8 b2 = *buf & 0xf;
			b = b2 + ((*buf >> 4) & 0xf); 	// b代表 07H 00 02的结束处 到31H之间的长度
			if (buf + b > attr_end) { goto io_error; } // 参数合法性检测

			// 这个循环是把 "运行的起始簇号" 取出来,保存到deltaxcn中
...
// can investigate it further!
			...
			if ( vol->major_ver < 3 ) 
			{
				if ( deltaxcn == (LCN)-1 ) { ntfs_log_debug("lcn delta == -1\n"); }
				if ( lcn == (LCN)-1 ) { ntfs_log_debug("lcn == -1\n"); }
			}
#endif
			if (lcn < (LCN)-1) 
			{
				ntfs_log_debug("Invalid LCN < -1 in mapping pairs array.\n");
				goto err_out;
			}

			rl[rlpos].lcn = lcn; // ----- 记录3 -----
		}
		
		rlpos++; // Get to the next runlist element.

		//     低4位的内容  +  高4位的内容        + 1 --->下一个运行位置
		buf += (*buf & 0xf) + ((*buf >> 4) & 0xf) + 1;
	} // end-of-while

	if (buf >= attr_end) { goto io_error; }

	// 若该非常驻属性偏移+0x18的地方"结束的虚拟簇号"有值,则必须与总的VCN-1相等
	deltaxcn = *(s64*)( (char*)attr + 24 ) /*sle64_to_cpu(attr->highest_vcn)*/;
	if ( deltaxcn && vcn - 1 != deltaxcn ) {
mpa_err:
		ntfs_log_debug("Corrupt mapping pairs array in non-resident attribute.\n");
		goto err_out;
	}

	// Setup not mapped runlist element if this is the base extent.
//	if ( !attr->lowest_vcn ) // 起始的VCN为0,表明this is the base extent.
	if ( !*(VCN*)( (char*)attr + 16 ) )
	{
		VCN max_cluster;
	//	max_cluster = ( (attr->allocated_size + vol->cluster_size - 1) >> vol->cluster_size_bits ) - 1;
		max_cluster = ( (*(s64*)( (char*)attr + 40 ) + vol->cluster_size - 1) >> vol->cluster_size_bits ) - 1;
	
		if (deltaxcn) // 临时变量deltaxcn此时表示的是"结束的虚拟簇号"
		{
			//
			// If there is a difference between the highest_vcn and
			// the highest cluster, the runlist is either corrupt
			// or, more likely, there are more extents following this one.
			//
			if ( deltaxcn < max_cluster ) 
			{
				ntfs_log_debug(
					"More extents to follow; deltaxcn = 0x%llx, max_cluster = 0x%llx\n",
					(LONG64)deltaxcn,
					(LONG64)max_cluster );
		...
		// 若"结束的虚拟簇号"为0,表明this is a single extent
	...
	if (!old_rl) 
	{
		...
		ntfs_debug_runlist_dump( rl ); // 调试时打印信息
		return rl;
	}

	// 合并新旧run list
	old_rl = ntfs_runlists_merge( old_rl, rl );
	if (old_rl) { return old_rl; }
...
Author: sudami [sudami@163.com]
Time  : 2009/03/02 [2:3:2009 - 12:57]

Routine Description:
  在run list中,把vcn转换成对应的LCN  
...
Return Value:
  ...
  ==================================================
   ...
   -2 = LCN_RL_NOT_MAPPED	This is part of the runlist which has not been inserted into the runlist yet.
...
--*/
{
	...
	if (vcn < (VCN)0) { return (LCN)LCN_EINVAL; }
	if (!rl) { return (LCN)LCN_RL_NOT_MAPPED; }
	if (vcn < rl[0].vcn) { return (LCN)LCN_ENOENT; }
	...
	if (rl[i].lcn < (LCN)0) { return rl[i].lcn; }
...
ntfs_get_size_for_mapping_pairs(
	...
	IN const runlist_element *rl, 
	IN const VCN start_vcn
	)
/*++

Author: sudami [sudami@163.com]
Time  : 2009/03/06 [6:3:2009 - 15:34]
...
--*/
{
	...
	while (rl->length && start_vcn >= rl[1].vcn) { rl++; }
	...
	for (; rl->length; rl++) 
	{
		if (rl->length < 0 || rl->lcn < LCN_HOLE) { goto err_out; }
		/* Header byte + length. */
...
static runlist_element * test_rl_pure_src(BOOL contig, BOOL multi, int vcn, int len)
{
	...
	result = (runlist_element *) ntfs_malloc(4096);
	if (!result)
		return NULL;
...
static void test_rl_pure_test(int test, BOOL contig, BOOL multi, int vcn, int len, runlist_element *file, int size)
{
	...
	dst = (runlist_element *) ntfs_malloc(4096);
	if (!src || !dst) {
...
{
	ntfs_volume vol;
	ATTR_RECORD *attr1 = (ATTR_RECORD *) ntfs_malloc(1024);
	ATTR_RECORD *attr2 = (ATTR_RECORD *) ntfs_malloc(1024);
	ATTR_RECORD *attr3 = (ATTR_RECORD *) ntfs_malloc(1024);
