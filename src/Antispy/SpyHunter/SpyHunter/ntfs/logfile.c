		...
		LSN *lsn)
{
	 ...
	 */
	trp = (RESTART_PAGE_HEADER *) ntfs_malloc(le32_to_cpu(rp->system_page_size));
	if (!trp)
		return errno;
	/*
...
ntfs_check_logfile(
	IN ntfs_attr *log_na, 
	OUT RESTART_PAGE_HEADER **rp
	)
/*++

Author: sudami [sudami@163.com]
Time  : 2009/03/03 [3:3:2009 - 20:12]
...
Arguments:
  ...
  rp - on success this is a copy of the current restart page
...
--*/
{
	...
	//
	if (size < log_page_size * 2 
		|| (size - log_page_size * 2) >> log_page_bits < MinLogRecordPages
	   ) 
	{
	...
	// 为restart page分配内存
	kaddr = (u8 *) ntfs_malloc(NTFS_BLOCK_SIZE);
	if (!kaddr)
		return FALSE;
	/*
