 ...
 */
int ntfs_sid_to_mbs_size(const X_SID *sid)
{
	int size, i;
 ...
 * set to the error code.
 */
char *ntfs_sid_to_mbs(const X_SID *sid, char *sid_str, size_t sid_str_size)
{
	u64 u;
	char *s;
	int i, j, cnt;
	...
	/* Start with "S-R-". */
	i = _snprintf(s, cnt, "S-%hhu-", (unsigned char)sid->revision);
	if (i < 0 || i >= cnt)
		goto err_out;
	s += i;
	cnt -= i;
	...
	if (!sid->identifier_authority.high_part)
		i = _snprintf(s, cnt, "%lu", (unsigned long)u);
	else
		i = _snprintf(s, cnt, "0x%llx", (ULONG64)u);
	if (i < 0 || i >= cnt)
		goto err_out;
	s += i;
	cnt -= i;
	...
	for (j = 0; j < sid->sub_authority_count; j++) {
		i = _snprintf(s, cnt, "-%u", (unsigned int)
...
int ntfs_sd_add_everyone(ntfs_inode *ni)
{
	...
	sid = (X_SID *)((u8 *)sd + sizeof(SECURITY_DESCRIPTOR_ATTR));
	sid->revision = 1;
	...
	acl = (ACL *)((u8 *)sid + sizeof(X_SID) + 4);
	acl->AclRevision = 2;
	acl->AclSize = cpu_to_le16(sizeof(ACL) + sizeof(ACCESS_ALLOWED_ACE));
	...
	ace->Header.AceSize = cpu_to_le16(sizeof(ACCESS_ALLOWED_ACE));
	ace->Mask = cpu_to_le32(0x1f01ff); /* FIXME */

	pTmpSID = (X_SID*)ace->SidStart ;
	if ( pTmpSID )
	{
		pTmpSID->revision = 1;
		pTmpSID->sub_authority_count = 1;
		pTmpSID->sub_authority[0] = 0;
		pTmpSID->identifier_authority.value[5] = 1;
	}
...
// 	ace->sid.identifier_authority.value[5] = 1;
	...
	ret = ntfs_attr_add(ni, AT_SECURITY_DESCRIPTOR, AT_UNNAMED, 0, (u8 *)sd, sd_len);
	if (ret)
