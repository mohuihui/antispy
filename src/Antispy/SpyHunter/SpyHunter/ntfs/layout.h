...
#pragma warning( disable: 4200 4341 4309 )
...
typedef struct {
	...
	u8  sectors_per_cluster;	// 每簇扇区数(1簇 = 8扇区)
	u16 reserved_sectors;		/* zero */
	u8  fats;					/* zero */
	u16 root_entries;			/* zero */
	u16 sectors;				/* zero */
	u8  media_type;	// 介质描述;0xf8 = hard disk 
	u16 sectors_per_fat;		/* zero */
/*0x0d*/u16 sectors_per_track;	/* Required to boot Windows. */
/*0x0f*/u16 heads;				/* Required to boot Windows. */
/*0x11*/u32 hidden_sectors;	    // 隐含扇区	
/* Offset to the start of the partition relative to the disk in sectors.
...
/*0x0b*/BIOS_PARAMETER_BLOCK bpb;	/* 指向神奇的BPB结构 */
...
typedef enum {
	...
	magic_BAAD = const_cpu_to_le32(0x44414142), 
	/* Failed multi sector transfer was detected. */
	 ...
	 * it.
	 */
	magic_empty = const_cpu_to_le32(0xffffffff)
	/* 记录是空的,在使用它之前必须初始化. */
} NTFS_RECORD_TYPES;
...
// 	MFT_REC_SPACE_FILLER		= const_cpu_to_le16(0xffff) /* Just to make flags 16-bit. */
// }  MFT_RECORD_FLAGS;

#define MFT_RECORD_FLAGS u16
...
#define MFT_REC_SPACE_FILLER		/*const_cpu_to_le16*/(0xffff)
 ...
 */
#define MFT_REF_MASK_CPU 0x0000ffffffffffff
...
/*Ofs*/
/*  0	NTFS_RECORD;  */
	NTFS_RECORD_TYPES magic; /* Usually the magic is "FILE". */ // MFT标志位
	u16 usa_ofs;		// 更新序列号的偏移 
	u16 usa_count;		// 更新序列号的大小与数组
...
/* 18*/	u16 link_count;	// 硬连接数,即有多少目录指向该文件
...
/* 20*/	u16 attrs_offset; // 第一个属性的偏移
/* 22*/	MFT_RECORD_FLAGS flags;	 // 文件被删除后FLAG=0
/* 24*/	u32 bytes_in_use; // 这个文件记录中已用的大小
/* 28*/	u32 bytes_allocated;	// 和一个MFT的大小相同.那就是1KB咯

/* 32*/	MFT_REF base_mft_record; // 基本文件记录中的文件索引号
/* This is zero for base mft records.
				   ...
				   mft record). */
/* 40*/	u16 next_attr_instance;  // 下一属性ID
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//     --     -      -     -- 
//+                                                           +//     --      -   -       -- 
//+         神奇的NTFS卷属性.eg:30H,80H,90H,A0H,B0H           +//      --       -        --  
//+                                                           +//       -     sudami     -   
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//        --            --    
/////////////////////////////////////////////////////////////////          --        --  
//                                                                           --    --
...
typedef enum {
	...
	AT_STANDARD_INFORMATION	= const_cpu_to_le32(0x10), // 标准信息
	AT_ATTRIBUTE_LIST		= const_cpu_to_le32(0x20), // 属性列表
	AT_FILE_NAME			= const_cpu_to_le32(0x30), // 文件名
	AT_OBJECT_ID			= const_cpu_to_le32(0x40), // 对象ID
	AT_SECURITY_DESCRIPTOR	= const_cpu_to_le32(0x50), // 安全描述符
	AT_VOLUME_NAME			= const_cpu_to_le32(0x60), // 卷名
	AT_VOLUME_INFORMATION	= const_cpu_to_le32(0x70), // 卷标志
	AT_DATA					= const_cpu_to_le32(0x80), // 文件数据
	AT_INDEX_ROOT			= const_cpu_to_le32(0x90), // 索引根目
	AT_INDEX_ALLOCATION		= const_cpu_to_le32(0xa0), // 索引分配
	AT_BITMAP				= const_cpu_to_le32(0xb0), // 位图
	AT_REPARSE_POINT		= const_cpu_to_le32(0xc0), // 重解析点
	AT_EA_INFORMATION		= const_cpu_to_le32(0xd0), // 扩充属性信息
	AT_EA					= const_cpu_to_le32(0xe0), // 扩充属性
	AT_PROPERTY_SET			= const_cpu_to_le32(0xf0), // 
	AT_LOGGED_UTILITY_STREAM	= const_cpu_to_le32(0x100), // EFS加密属性
 ...
 *	security_id (u32).
 * COLLATION_NTOFS_SID - Sorting is done according to ascending x_SID values.
...
//
// 这里的标志信息是位于MFT属性头中的那个,和文件本身的属性标志不同:
// 
// 文件本身的属性标志如下:
// 0001H -- 只读文件; 0002H -- 隐含文件
// 0004H -- 系统文件; 0020H -- 存档文件
// ...
//

//
// 不能用枚举类型,因为VC6编译器会始终认为enum类型的是32位的,即使里面的内容是
// 16位的,也会被扩充为32的
//
// typedef enum {
// 	ATTR_IS_COMPRESSED	= const_cpu_to_le16(0x0001), // 压缩的
// 	ATTR_COMPRESSION_MASK	= const_cpu_to_le16(0x00ff), 
// 	/* Compression method mask. Also, first illegal value. */
// 	ATTR_IS_ENCRYPTED	= const_cpu_to_le16(0x4000), // 加密的
// 	ATTR_IS_SPARSE		= const_cpu_to_le16(0x8000) // 稀疏的
// }  ATTR_FLAGS;

#define ATTR_FLAGS u16

#define ATTR_IS_COMPRESSED		/*const_cpu_to_le16*/(0x0001)		// 压缩的
#define ATTR_COMPRESSION_MASK	/*const_cpu_to_le16*/(0x00ff)
// Compression method mask. Also, first illegal value.
#define ATTR_IS_ENCRYPTED		/*const_cpu_to_le16*/(0x4000)		// 加密的
#define ATTR_IS_SPARSE			/*const_cpu_to_le16*/(0x8000)		// 稀疏的
...
/* 14*/	u16 instance;		/* The instance of this attribute record. This
	...
	// 因为是4种属性头,在这上面都是相同的,下面这个union就分不同的情况了...
...
/* 16*/ union {
		...
		/* Resident attributes. */
		struct {
/* 16 */		u32 value_length; // 属性的长度,不包含属性头
/* 20 */		u16 value_offset; // 同name_offset
/* 22 */		RESIDENT_ATTR_FLAGS resident_flags; // 索引标志
...
/* 24*/			VCN highest_vcn; /* Highest valid vcn of this extent of
				...
				attributes. */
/* 32*/			u16 mapping_pairs_offset; // 数据运行信息的偏移地址
			// Run是一个在逻辑簇号(LCN)上连续的区域,不存储在MFT中的数据
...
/* 34*/			u8 compression_unit; // 压缩单位大小,0表示未压缩
...
/* 56*/			s64 initialized_size;	/* Byte size of initialized
				...
				data_size. */
/* 64 */		void *non_resident_end[1]; /* Use offsetof(ATTR_RECORD,
...
/*  8*/	s64 last_data_change_time;	
/* 16*/	s64 last_mft_change_time;
...
/*  8*/	VCN lowest_vcn;		/* Lowest virtual cluster number of this portion
				   ...
				   as signed. */
/* 16*/	MFT_REF mft_reference;	// 属性的基本文件参考号
/* The reference of the mft record holding
				   the ATTR_RECORD for this portion of the attribute value. */
...
#define FILE_NAME_TYPE_FLAGS u8
...
/*  8*/	s64 creation_time;		
/* 10*/	s64 last_data_change_time;	
/* 18*/	s64 last_mft_change_time;
/* 20*/	s64 last_access_time;		
/* 28*/	s64 allocated_size;	// 文件分配大小
/* 30*/	s64 data_size;		// 文件实际大小
...
/**
 * struct x_GUID - x_GUID structures store globally unique identifiers (x_GUID).
 *
 * A x_GUID is a 128-bit value consisting of one group of eight hexadecimal
...
#pragma pack(1)
typedef struct {
	u32 data1;	/* The first eight hexadecimal digits of the x_GUID. */
...
typedef struct {
	...
	union {
		struct {
			x_GUID birth_volume_id;
			x_GUID birth_object_id;
			x_GUID domain_id;
		} ;
		u8 extended_info[48];
	} ;
...
#pragma pack(1)
typedef struct {
	x_GUID object_id;				/* Unique id assigned to the
						   file.*/
	/* The following fields are optional. The attribute value size is 16
	   bytes, i.e. sizeof(x_GUID), if these are not present at all. Note,
	...
	union {
		struct {
			x_GUID birth_volume_id;	/* Unique id of volume on which
						   the file was first created.*/
			x_GUID birth_object_id;	/* Unique id of file when it was
						   first created. */
			x_GUID domain_id;		/* Reserved, zero. */
		} ;
		u8 extended_info[48];
	} ;
}  OBJECT_ID_ATTR;
...
/**
 * union x_SID_IDENTIFIER_AUTHORITY - A 48-bit value used in the x_SID structure
 *
...
}  x_SID_IDENTIFIER_AUTHORITY;
#pragma pack()

/**
 * struct x_SID -
 *
 * The x_SID structure is a variable-length structure used to uniquely identify
 * users or groups. x_SID stands for security identifier.
 *
 * The standard textual representation of the x_SID is of the form:
 *	S-R-I-S-S...
 * Where:
 *    - The first "S" is the literal character 'S' identifying the following
 *	digits as a x_SID.
 *    - R is the revision level of the x_SID expressed as a sequence of digits
 *	in decimal.
 ...
 *	decimal.
 *
 * Example x_SID; the domain-relative x_SID of the local Administrators group on
 * Windows NT/2k:
 *	S-1-5-32-544
 * This translates to a x_SID with:
 *	revision = 1,
...
typedef struct {
	...
	x_SID_IDENTIFIER_AUTHORITY identifier_authority;
 ...
 * (security_id, not to be confused with a x_SID). The security_id is unique for
...
#define VOLUME_CHKDSK_UNDERWAY		(0x4000)
#define VOLUME_MODIFIED_BY_CHKDSK	(0x8000)
#define VOLUME_FLAGS_MASK			(0xc03f)


/////////////////////////////////////////////////////////////////         --          --     
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//     --     -      -     -- 
//+                                                           +//     --      -   -       -- 
//+                属性 70H   VOLUME_INFORMATION              +//      --       -        --  
//+                                                           +//       -     sudami     -   
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//        --            --    
/////////////////////////////////////////////////////////////////          --        --  
//                                                                           --    --
...
// typedef enum _INDEX_HEADER_FLAGS_ {
 ...
 * a quota on that volume. The index key holds the x_SID of the user_id the
 * entry belongs to, i.e. the owner_id. The collation rule for $O is
 * COLLATION_NTOFS_SID.
 *
 * The $O index entry data is the user_id of the user corresponding to the x_SID.
 * This user_id is used as an index into $Q to find the quota control entry
 * associated with the x_SID.
 *
...
/* The below field is NOT present for the quota defaults entry. */
	SID sid;		/* The x_SID of the user/object associated with
 ...
 */
#pragma pack(1)
typedef enum _PREDEFINED_OWNER_IDS_ {
...
// typedef enum _INDEX_ENTRY_FLAGS_ {
...
#define INDEX_ENTRY_FLAGS u16

#define INDEX_ENTRY_NODE			1
#define INDEX_ENTRY_END				2
#define INDEX_ENTRY_SPACE_FILLER	0xffff
...
/* 16*/	union {		/* The key of the indexed attribute. NOTE: Only present
		...
		x_GUID object_id;		/* $O index in FILE_Extend/$ObjId: The
					   object_id of the mft record found in
					   the data part of the index. */
		REPARSE_INDEX_KEY reparse;	/* $R index in
						   FILE_Extend/$Reparse. */
		x_SID sid;		/* $O index in FILE_Extend/$Quota:
					   x_SID of the owner of the user_id. */
 ...
 */
/*
typedef enum _PREDEFINED_REPARSE_TAGS_ {
 ...
 * x_SID, an optional x_GUID, an optional container name, a non-optional user name,
...
/* 16*/	u8 unknown4[16];	/* MD5 hash of decrypted FEK?  This field is
				   ...
				   likely to be x_GUID of this encrytped file
...
// typedef enum _INTX_FILE_TYPES_ {
// 	INTX_SYMBOLIC_LINK = const_cpu_to_le64(0x014B4E4C78746E49), /* "IntxLNK\1" */
// 	INTX_CHARACTER_DEVICE = const_cpu_to_le64(0x0052484378746E49), /* "IntxCHR\0" */
// 	INTX_BLOCK_DEVICE = const_cpu_to_le64(0x004B4C4278746E49) /* "IntxBLK\0" */
// } INTX_FILE_TYPES;

#define INTX_FILE_TYPES u64

#define INTX_SYMBOLIC_LINK		const_cpu_to_le64(0x014B4E4C78746E49) /* "IntxLNK\1" */
#define INTX_CHARACTER_DEVICE	const_cpu_to_le64(0x0052484378746E49) /* "IntxCHR\0" */
#define INTX_BLOCK_DEVICE		const_cpu_to_le64(0x004B4C4278746E49) /* "IntxBLK\0" */
...
#pragma pack(1)
typedef struct _X_SID_IDENTIFIER_AUTHORITY_ {
	union{
		struct {
			u16 high_part;		/* High 16-bits. */
			u32 low_part;		/* Low 32-bits. */
