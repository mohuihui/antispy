...
int g_current_drive_id = 0 ; // 当前正在操作的文件的分区号
...
{
	liDistanceToMove.LowPart = SetFilePointer(
									hFile,
									liDistanceToMove.LowPart, 
									&liDistanceToMove.HighPart,
									dwMoveMethod );
 ...
 */
void ntfs_device_win32_init_imports(void)
{
 ...
 */
int 
ntfs_device_win32_simple_open_file(
...
// 			NULL, OPEN_EXISTING, 0, NULL);
	...
	*handle = CreateFileA( filename, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_WRITE | FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL );
 ...
 */
int ntfs_device_win32_lock(HANDLE handle)
{
	DWORD i;
 ...
 *	 -1 if not, and errno set.
 */
int ntfs_device_win32_unlock(HANDLE handle)
{
	DWORD i;
 ...
 *	 -1 if not, and errno set.
 */
int ntfs_device_win32_dismount(HANDLE handle)
{
	DWORD i;
 ...
 *	 -1 if not, and errno set.
 */
s64 ntfs_device_win32_getsize(HANDLE handle)
{
	DWORD loword, hiword;
 ...
 *	 -1 if not, and errno set.
 */
s64 ntfs_device_win32_getdisklength(HANDLE handle)
{
 ...
 *	 -1 if not, and errno set.
 */
s64 ntfs_device_win32_getntfssize(HANDLE handle)
{
	s64 rvl;
 ...
 */
int ntfs_device_win32_getgeo(HANDLE handle, win32_fd *fd)
{
	DWORD i;
	BOOL rvl;
	BYTE b[sizeof(DISK_GEOMETRY) + sizeof(DISK_PARTITION_INFO) + sizeof(DISK_DETECTION_INFO) + 512];
	...
	//	ntfs_log_debug("GET_DRIVE_GEOMETRY_EX detected.\n");
		DISK_DETECTION_INFO *ddi = (PDISK_DETECTION_INFO)
				...
				( ( (PDISK_PARTITION_INFO)(&((PDISK_GEOMETRY_EX)b)->Data) )->SizeOfPartitionInfo )
				);
 ...
 *	 -1 if not, and errno set.
 */
int ntfs_device_win32_open_file(char *filename, win32_fd *fd,
		int flags)
{
	HANDLE handle;
 ...
 *        -1 if not, and errno set.
 */
int ntfs_device_win32_open_drive(int drive_id, win32_fd *fd, int flags)
{
	HANDLE handle;
	int err;
	...
	if ((err = ntfs_device_win32_simple_open_file( filename, &handle, flags, TRUE ))) {
		return err;
	}
 ...
 */
HANDLE ntfs_device_win32_open_volume_for_partition(unsigned int drive_id,
		s64 part_offset, s64 part_length, int flags)
{
	...
	do {
		...
		handle = CreateFile(vol_name,
				GENERIC_READ | GENERIC_WRITE /*ntfs_device_unix_status_flags_to_win32(flags)*/,
...
#define EXTENTS_SIZE sizeof(VOLUME_DISK_EXTENTS) + 9 * sizeof(DISK_EXTENT)
					...
					&bytesReturned, NULL)) 
			{
				...
				{
					DISK_EXTENT *extent = &((VOLUME_DISK_EXTENTS *)extents)->Extents[0] ;
 ...
 */
BOOL 
ntfs_device_win32_find_partition(
	IN HANDLE handle,
	IN DWORD partition_id,
	OUT s64 *part_offset,
	OUT s64 *part_length,
	OUT int *hidden_sectors
	)
{
	...
	part_count = 8;
	do {
		buf_size = sizeof(DRIVE_LAYOUT_INFORMATION) + part_count * sizeof(PARTITION_INFORMATION);
		...
		if (DeviceIoControl(handle, IOCTL_DISK_GET_DRIVE_LAYOUT, NULL, 0, (BYTE*)drive_layout, buf_size, &i, NULL))
			break;
		...
		if (part_count > 512) 
		{
			ntfs_log_trace("GetDriveLayout failed: More than 512 partitions?\n");
			errno = ENOBUFS; 
			return FALSE;
		}
	...
	for (i = 0; i < drive_layout->PartitionCount; i++) 
	{
		...
		{
			*part_offset = drive_layout->PartitionEntry[i].StartingOffset.QuadPart;
			*part_length = drive_layout->PartitionEntry[i].PartitionLength.QuadPart;
	...
	)
{
	...
	/*	Open the entire device without locking, ask questions later  
		通过Win32 API 打开磁盘设备
	*/
	if ((err = ntfs_device_win32_simple_open_file(drive_name, &handle, flags, FALSE))) {
		return err;
	}

	if (ntfs_device_win32_find_partition(handle, partition_id, &part_start, &part_length, &hidden_sectors))
	{
		s64 tmp;
		HANDLE vol_handle = ntfs_device_win32_open_volume_for_partition( drive_id, part_start, part_length, flags );
		/* Store the drive geometry. */
		ntfs_device_win32_getgeo(handle, fd);
		...
		if (vol_handle != INVALID_HANDLE_VALUE) {
			if (((flags & O_RDWR) == O_RDWR) && ntfs_device_win32_lock(vol_handle)) 
			{
 ...
 */
int ntfs_device_win32_open(struct ntfs_device *dev, int flags)
{
	...
	sscanf(dev->d_name, "%c%u", &drive_char, &part);
	drive_id = toupper(drive_char) - 'C';

	if ( strstr( dev->d_name, "//" ) ) {
		numparams = 0 ; // 是文件
 ...
 */
s64 ntfs_device_win32_seek(struct ntfs_device *dev, s64 offset, int whence)
{
	s64 abs_ofs;
 ...
 */
s64 ntfs_device_win32_pio(win32_fd *fd, const s64 pos,
 ...
 */
s64 
ntfs_device_win32_pread_simple(
	win32_fd *fd, 
	const s64 pos,
	const s64 count, 
	void *b
 ...
 */
s64 ntfs_device_win32_read(struct ntfs_device *dev, void *b, s64 count)
{
 ...
 */
int ntfs_device_win32_close(struct ntfs_device *dev)
{
	win32_fd *fd = (win32_fd *)dev->d_private;
	BOOL rvl;
 ...
 */
int ntfs_device_win32_sync(struct ntfs_device *dev)
{
	int err = 0;
	BOOL to_clear = TRUE;
 ...
 * On error returns -1 with errno set.
 */
s64 ntfs_device_win32_write(struct ntfs_device *dev, const void *b, s64 count)
{
	...
	if (fd->vol_handle != INVALID_HANDLE_VALUE && old_pos < fd->geo_size) {
		...
		if (count > vol_to_write) {
			...
			bw -= ofs;
			fd->pos += bw;
			ofs = (int)(fd->pos & (NTFS_BLOCK_SIZE - 1)) ;
			if (bw != vol_to_write)
				goto write_partial;
		}
	}
 ...
 */
int ntfs_win32_hdio_getgeo(struct ntfs_device *dev,
		struct hd_geometry *argp)
{
	win32_fd *fd = (win32_fd *)dev->d_private;

	argp->heads = (UCHAR) fd->geo_heads;
	argp->sectors = (UCHAR) fd->geo_sectors;
	argp->cylinders = (USHORT) fd->geo_cylinders;
	argp->start = (ULONG) fd->part_hidden_sectors;
	return 0;
}
 ...
 *	 -1 if not, and errno set.  Note if error fd->handle is trashed.
 */
int ntfs_win32_blksszget(struct ntfs_device *dev,int *argp)
{
	win32_fd *fd = (win32_fd *)dev->d_private;
	DWORD bytesReturned;
	DISK_GEOMETRY dg;
...
int ntfs_device_win32_ioctl(struct ntfs_device *dev, int request, void *argp)
{
	win32_fd *fd = (win32_fd *)dev->d_private;
...
s64 CalcSectorCounts(s64 offset, s64* count)
{
	WORD wSectors = 0 ;
	WORD wsssssss = 0 ;
	s64 littleOffset = 0 ;
	s64 offsetTmp = offset ;

	littleOffset = offsetTmp % 512 ; // 得到offset偏移和一个扇区求余 的剩余量

	offsetTmp -= offsetTmp % 512 ; // 计算出该读取的偏移(512对齐)

	*count += littleOffset ; // 得到要读取的总数量

	// 计算出要读取的扇区数
	wsssssss = ((DWORD)*count % 512) > 0 ? 1 : 0 ;
	wSectors =  (DWORD)*count / 512 + wsssssss ;
...
s64 ntfs_device_win32_pread(struct ntfs_device *dev, void *b, s64 count, s64 offset)
{
	DWORD dwCB, dwReturn; 
	s64 FinalOffset ;
	BOOL bRet = FALSE, bResult = FALSE ;
	PVOID pTmpBuffer = NULL ;

	//
	// offset必须是一个扇区大小(0x200 - 512 )的倍数. 所以在SetFilePointer之前要校准一下
	//
	FinalOffset = CalcSectorCounts( offset, &count );

	//
	// 开始读取...
	//

	pTmpBuffer = (PVOID) malloc(count);
	if ( NULL == pTmpBuffer ) { return 0 ; }
	memset( pTmpBuffer, 0, count );

	// 在驱动中用SCSI命令读扇区
	bResult = read_sector_ex( pTmpBuffer, (ULONG)(FinalOffset / 0x200), (ULONG) (count / 0x200), *(char*)dev->d_name);
	...
	dwCB = (DWORD)count ; // 让其成功返回

	// 按需求拷贝到 @b中
	memcpy( (char*)b, (char*)pTmpBuffer, (DWORD)count );
	free(pTmpBuffer);
...
s64 ntfs_device_win32_pwrite(struct ntfs_device *dev, const void *b, s64 count, s64 offset)
{
	win32_fd* tmpFD ;
	HANDLE hDriver ;
	DWORD dwCB, dwReturn ;
	...
	// 获取句柄
	tmpFD = (win32_fd*) dev->d_private ;
	hDriver = (HANDLE) tmpFD->vol_handle ;
	if ( !hDriver ) { return 0; }

	FinalOffset = CalcSectorCounts( offset, &count );
	
	dwReturn = SetFilePointer( hDriver, (LONG)FinalOffset, 0, FILE_BEGIN);
	if ( -1 == dwReturn ){ 
		return 0 ;
	}

	WriteFile( hDriver, b, (DWORD)count, &dwCB, NULL);
