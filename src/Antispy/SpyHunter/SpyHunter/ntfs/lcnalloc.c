		...
		LCN start_lcn, const NTFS_CLUSTER_ALLOCATION_ZONES zone)
{
	...
	/* Allocate memory. */
	buf = (u8 *) ntfs_malloc(NTFS_LCNALLOC_BSIZE);
	if (!buf)
		return NULL;
	/*
	...
	while (1) {
		ntfs_log_trace("Start of outer while loop: done_zones = 0x%x, "
				...
				(LONG64)zone_start, (LONG64)zone_end,
				(LONG64)bmp_initial_pos, (LONG64)bmp_pos,
				rlpos, rlsize);
		/* Loop until we run out of free clusters. */
		last_read_pos = bmp_pos >> 3;
		ntfs_log_trace("last_read_pos = 0x%llx.\n", (LONG64)last_read_pos);
		...
		ntfs_log_trace("Before inner while loop: buf_size = %i, lcn = "
				...
				buf_size, (LONG64)lcn, (LONG64)bmp_pos,
				need_writeback);
		while (lcn < buf_size && lcn + bmp_pos < zone_end) {
			...
			*byte |= bit;
			...
			ntfs_log_trace("Adding run (lcn 0x%llx, len 0x%llx), "
					...
					"0x%llx, rlpos = %i.\n",
					(LONG64)(lcn + bmp_pos), 1,
					(LONG64)prev_lcn, (LONG64)lcn,
					(LONG64)bmp_pos,
					(LONG64)prev_run_len, rlpos);
			if (prev_lcn == lcn + bmp_pos - prev_run_len && rlpos) {
				ntfs_log_trace("Coalescing to run (lcn 0x%llx, len "
						"0x%llx).\n",
						(LONG64)rl[rlpos - 1].lcn,
						(LONG64) rl[rlpos - 1].length);
...
int ntfs_cluster_free_from_rl(ntfs_volume *vol, runlist *rl)
{
	...
	for (; rl->length; rl++) 
	{
		...
		ntfs_log_trace("Dealloc lcn 0x%llx, len 0x%llx.\n", (LONG64)rl->lcn, (LONG64)rl->length);

		if (rl->lcn >= 0 && ntfs_bitmap_clear_run(vol->lcnbmp_na, rl->lcn, rl->length)) 
		{
			int eo = errno;
...
int ntfs_cluster_free(ntfs_volume *vol, ntfs_attr *na, VCN start_vcn, s64 count)
{
	...
	ntfs_log_trace("Entering for inode 0x%llx, attr 0x%x, count 0x%llx, "
		       ...
		       na->type, (LONG64)count, (LONG64)start_vcn);
	...
	if (rl->lcn != LCN_HOLE) {
		...
		if (ntfs_bitmap_clear_run(vol->lcnbmp_na, rl->lcn + delta, to_free))
			return -1;
	...
	for (; rl->length && count != 0; ++rl) {
		...
		if (rl->lcn < 0 && rl->lcn != LCN_HOLE) {
			...
			ntfs_log_trace("Eeek! invalid lcn (= %lli).  Should attempt to map runlist!  Leaving inconsistent metadata!\n", (LONG64)rl->lcn);
			errno = EIO;
			return -1;
		}
		...
		if (rl->lcn != LCN_HOLE) {
			...
			if (ntfs_bitmap_clear_run(vol->lcnbmp_na, rl->lcn, to_free)) {
				...
				// FIXME: Eeek! We need rollback! (AIA)
				ntfs_log_trace("Eeek!  bitmap clear run failed. Leaving inconsistent metadata!\n");
				errno = eo;
				return -1;
			}
			/* We have freed @to_free real clusters. */
	...
	if (count != -1 && count != 0) {
		// FIXME: Eeek! BUG()
		ntfs_log_trace("Eeek!  count still not zero (= %lli).  Leaving inconsistent metadata!\n", (LONG64)count);
		errno = EIO;
		return -1;
	}
