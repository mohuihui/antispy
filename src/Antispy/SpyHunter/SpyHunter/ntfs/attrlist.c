...
int ntfs_attrlist_entry_add(ntfs_inode *ni, ATTR_RECORD *attr)
{
	...
	if (!ntfs_attr_lookup(attr->type, (attr->name_length) ? (ntfschar*)
			...
			(attr->non_resident) ? le64_to_cpu(attr->X.lowest_vcn) :
			0, (attr->non_resident) ? NULL : ((u8*)attr +
			le16_to_cpu(attr->X.value_offset)), (attr->non_resident) ?
			0 : le32_to_cpu(attr->X.value_length), ctx)) {
		/* Found some extent, check it to be before new extent. */
		if (ctx->al_entry->lowest_vcn == attr->X.lowest_vcn) {
			err = EEXIST;
	...
	if (attr->non_resident)
		ale->lowest_vcn = attr->X.lowest_vcn;
	else
		ale->lowest_vcn = 0;
...
int ntfs_attrlist_entry_rm(ntfs_attr_search_ctx *ctx)
{
	...
	ntfs_log_trace("Entering for inode 0x%llx, attr 0x%x, lowest_vcn %lld.\n",
			...
			(ULONG64) le32_to_cpu(ctx->al_entry->type),
			(LONG64) le64_to_cpu(ctx->al_entry->lowest_vcn));
