...
// extern LIST_ENTRY TempProcessListHead;				// 进程链表头
// extern KSPIN_LOCK TempProcessSpinLock;				// 进程链锁
...
// Returns:   VOID
// Qualifier: 初始化全局变量，一些结构偏移，Show ssdt的索引等。
...
//*************************************************
// Returns:   BOOL
// Qualifier: 通过AntiSpy驱动对象找到ntoskrnl的KLDR_DATA_TABLE_ENTRY
// Parameter: PDRIVER_OBJECT DriverObject - AntiSpy驱动对象
//*************************************************
...
BOOL InitWindows(PDRIVER_OBJECT DriverObject)
{
	...
	if (WinVersion != enumWINDOWS_UNKNOW && 
		GetKernelLdrDataTableEntry(DriverObject)) // 首先得到ntoskrnl.exe的KLDR_DATA_TABLE_ENTRY, 后面枚举啥的都用这个，防止我们的驱动被摘链了。
	{	
...
Routine Description:
	检查内存中的系统模块,(例如ntoskrnl.exe)是否和磁盘上的文件CheckSum是一致的.
	主要防止打过补丁的系统我们去读取原始的磁盘信息会有问题.

Arguments:
	nOldBase - 系统中的地址
	nMapedBase - 自己映射磁盘文件得到的地址

Return Value:
	STATUS_UNSUCCESSFUL - 不一致.
	STATUS_SUCCESS - 一致.
